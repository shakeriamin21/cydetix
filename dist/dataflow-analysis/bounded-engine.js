...
function appendControl(fact, control, step) {
    return {
        source: fact.source,
...
function xssTemplateContext(expression) {
    ...
    if (lower.lastIndexOf("<script") > lower.lastIndexOf("</script"))
        return "JAVASCRIPT";
    if (/(?:href|src|action|formaction)\s*=\s*["'][^"']*$/iu.test(prefix))
        return "URL";
    if (/[A-Za-z_:][-A-Za-z0-9_:.]*\s*=\s*["'][^"']*$/u.test(prefix))
        return "HTML_ATTRIBUTE";
    return "HTML_BODY";
}
function xssControlEvaluation(fact, context) {
    const controls = fact.controls ?? [];
    if (controls.length === 0)
        return "ABSENT";
    const effective = controls.some((control) => (control === "HTML_ESCAPE" &&
...
function redirectPolicyEnforced(source, destination, sinkOffset) {
    const name = rootIdentifier(destination);
    if (name === undefined)
        return false;
    const escaped = name.replaceAll(/[$]/g, "\\$");
    const before = source.slice(0, sinkOffset);
    const parsed = new RegExp(`(?:const|let|var)\\s+${escaped}\\s*=\\s*new\\s+URL\\s*\\(`).test(before);
    const exactHostGuard = new RegExp(`if\\s*\\([^\\n]*!\\s*(?:ALLOWED_REDIRECT_HOSTS|allowedRedirectHosts|allowed_redirect_hosts)\\.has\\s*\\(\\s*${escaped}\\.hostname\\s*\\)[^\\n]*\\)[^\\n]*(?:return|throw)`).test(before);
    const exactOriginGuard = new RegExp(`if\\s*\\([^\\n]*${escaped}\\.origin\\s*!==\\s*(?:["'][^"']+["']|APP_ORIGIN|appOrigin)[^\\n]*\\)[^\\n]*(?:return|throw)`).test(before);
    const schemeGuard = new RegExp(`if\\s*\\([^\\n]*${escaped}\\.protocol\\s*!==\\s*["']https?:["'][^\\n]*\\)[^\\n]*(?:return|throw)`).test(before);
    return parsed && ((exactHostGuard && schemeGuard) || exactOriginGuard);
}
function naiveRedirectPrefixCheck(source, destination, sinkOffset) {
    const name = rootIdentifier(destination);
    if (name === undefined)
        return false;
    const escaped = name.replaceAll(/[$]/g, "\\$");
    return new RegExp(`${escaped}\\.startsWith\\s*\\(\\s*["']/["']\\s*\\)`).test(source.slice(0, sinkOffset));
}
...
function createCandidate(kind, file, sinkNode, fact, sinkLabel, message, invariant, controlEvaluation) {
    if (typeof sinkNode.start !== "number" || typeof sinkNode.end !== "number")
        return undefined;
...
function analyzeJavaScriptFile(file, parsed, metrics, unknowns) {
    ...
    const shadowedImportNames = new Set();
        ...
        VariableDeclarator(path_) {
            ...
            if (initializer?.type === "CallExpression" &&
                (initializer.callee.type === "MemberExpression" ||
                    initializer.callee.type === "OptionalMemberExpression")) {
                const chain = memberChain(initializer.callee);
                if (chain?.root === "document" &&
                    ["getElementById", "querySelector", "querySelectorAll", "createElement"].includes(chain.properties.at(-1) ?? ""))
    ...
    const routeFrameworks = new Map();
    const markRouteArgument = (argument, framework) => {
    ...
    const recognizedHtmlControl = (expression) => {
        const chain = memberChain(expression.callee);
        if (expression.callee.type === "Identifier") {
            if (shadowedImportNames.has(expression.callee.name))
                return undefined;
        ...
        if (chain === undefined || shadowedImportNames.has(chain.root))
            return undefined;
        const binding = imports.get(chain.root);
        const method = chain.properties.at(-1);
        if (binding?.module === "he" && ["escape", "encode"].includes(method ?? ""))
            return "HTML_ESCAPE";
        if (["dompurify", "isomorphic-dompurify", "sanitize-html"].includes(binding?.module ?? "") &&
            method === "sanitize")
            return "HTML_SANITIZE";
        return undefined;
    };
    const evaluate = (scope, expression) => {
        ...
        if (expression.type === "CallExpression" || expression.type === "OptionalCallExpression") {
            ...
            const receiverFact = expression.callee.type === "MemberExpression" ||
    ...
    const recordUnknown = (ruleId, sinkNode, reasonCodes, explanation) => {
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration += 1) {
        ...
        traverse(parsed.ast, {
            ...
            AssignmentExpression(path_) {
                ...
                if (!staticallyUnreachable(path_) &&
                    ...
                    if (assignedFact.unknownControl) {
                        recordUnknown("AS-XSS-001", path_.node, ["SANITIZER_UNKNOWN", "INSUFFICIENT_DATAFLOW_PROOF"], "Untrusted browser content reaches DOM innerHTML through an unknown transformation.");
                    }
                    else {
                        const evaluation = xssControlEvaluation(assignedFact, "RAW_HTML");
                        if (evaluation !== "RECOGNIZED_EFFECTIVE") {
                            const candidate = createCandidate("XSS", file, path_.node, assignedFact, `DOM innerHTML assignment in ${info?.name ?? "top-level browser code"}`, "Proven attacker-influenced input reaches a structurally resolved DOM innerHTML sink.", "UNTRUSTED_BROWSER_CONTENT_MUST_NOT_REACH_RAW_RENDERING_WITHOUT_A_CONTEXT_APPROPRIATE_CONTROL", evaluation === "RECOGNIZED_INEFFECTIVE" ? "RECOGNIZED_INEFFECTIVE" : "ABSENT");
            ...
            JSXAttribute(path_) {
                if (path_.node.name.name !== "dangerouslySetInnerHTML")
                    return;
                ...
                if (rawHtml === undefined || fact === undefined)
                    return;
                if (fact.unknownControl) {
                    recordUnknown("AS-XSS-001", path_.node, ["SANITIZER_UNKNOWN", "INSUFFICIENT_DATAFLOW_PROOF"], "Untrusted content reaches React dangerouslySetInnerHTML through an unknown transformation.");
                    return;
                }
                const evaluation = xssControlEvaluation(fact, "RAW_HTML");
                if (evaluation === "RECOGNIZED_EFFECTIVE")
                    return;
                const candidate = createCandidate("XSS", file, path_.node, fact, `React ${opening.name.name}.dangerouslySetInnerHTML in ${info?.name ?? "route"}`, "Proven untrusted input reaches React's explicit raw HTML escape hatch without a supported HTML sanitizer.", "UNTRUSTED_BROWSER_CONTENT_MUST_NOT_REACH_RAW_RENDERING_WITHOUT_A_CONTEXT_APPROPRIATE_CONTROL", evaluation === "RECOGNIZED_INEFFECTIVE" ? "RECOGNIZED_INEFFECTIVE" : "ABSENT");
                if (candidate !== undefined)
                    candidates.set(candidate.fingerprintAnchor, candidate);
            },
            ...
            CallExpression(path_) {
                ...
                const explicitFastifyHtml = responseParameter !== undefined &&
                    new RegExp(`${responseParameter.replaceAll(/[$]/g, "\\$")}\\.(?:type|header)\\s*\\([^)]*["'](?:content-type["']\\s*,\\s*)?text/html`, "i").test(file.text.slice(functions.get(scope)?.node.start ?? 0, call.start ?? 0));
                const htmlResponseSink = descriptor.receiver === responseParameter &&
                ...
                if (htmlResponseSink && first !== undefined && firstFact !== undefined) {
                    ...
                    if (firstFact.unknownControl) {
                        recordUnknown("AS-XSS-001", call, ["SANITIZER_UNKNOWN", "INSUFFICIENT_DATAFLOW_PROOF"], "Untrusted content reaches an HTML response through a transformation whose context-specific safety is UNKNOWN.");
                    }
                    else if (evaluation !== "RECOGNIZED_EFFECTIVE") {
                        add(createCandidate("XSS", file, call, firstFact, `${routeFramework} ${responseParameter}.${descriptor.method} (${context}) in ${info?.name ?? "route"}`, `Proven untrusted input reaches a supported ${context} browser rendering sink without a context-appropriate control.`, "UNTRUSTED_BROWSER_CONTENT_MUST_NOT_REACH_RAW_RENDERING_WITHOUT_A_CONTEXT_APPROPRIATE_CONTROL", evaluation === "RECOGNIZED_INEFFECTIVE" ? "RECOGNIZED_INEFFECTIVE" : "ABSENT"));
                    }
                }
                const redirectBinding = call.callee.type === "Identifier"
                    ? importedBinding(imports, call.callee.name, REDIRECT_MODULES, path_)
                    : undefined;
                const responseRedirect = descriptor.receiver === responseParameter && descriptor.method === "redirect";
                const nextRedirect = (redirectBinding !== undefined && redirectBinding.imported === "redirect") ||
                ...
                const redirectArgument = responseRedirect && first?.type === "NumericLiteral" ? arguments_[1] : first;
                ...
                if (redirectSink && redirectArgument !== undefined && redirectFact !== undefined) {
                    if (redirectPolicyEnforced(file.text, redirectArgument, call.start ?? 0)) {
                        ...
                        recordUnknown("AS-REDIRECT-001", call, ["SANITIZER_UNKNOWN", "INSUFFICIENT_DATAFLOW_PROOF"], "An unknown destination transformation precedes a redirect sink; redirect policy sufficiency is UNKNOWN.");
                    }
                    else {
                        const weakPrefix = naiveRedirectPrefixCheck(file.text, redirectArgument, call.start ?? 0);
                        const proofFact = weakPrefix
                            ...
                            : redirectFact;
                        add(createCandidate("OPEN_REDIRECT", file, call, proofFact, `${responseRedirect ? routeFramework : "Next.js"} redirect in ${info?.name ?? "route"}`, "Proven attacker-influenced input determines a supported redirect destination without an explicit approved destination policy.", "UNTRUSTED_REDIRECT_DESTINATIONS_MUST_BE_CONFINED_TO_AN_EXPLICIT_APPROVED_POLICY", weakPrefix ? "RECOGNIZED_INEFFECTIVE" : "ABSENT"));
                    }
                }
            },
        });
...
function pythonXssContext(text) {
    ...
    if (prefix.lastIndexOf("<script") > prefix.lastIndexOf("</script"))
        return "JAVASCRIPT";
    if (/(?:href|src|action|formaction)\s*=\s*["'][^"']*$/u.test(prefix))
        return "URL";
    if (/[a-z_:][-a-z0-9_:.]*\s*=\s*["'][^"']*$/u.test(prefix))
        return "HTML_ATTRIBUTE";
    return "HTML_BODY";
}
function pythonRedirectPolicyEnforced(source, destination, sinkOffset) {
    const identifier = /^\s*(?:url\s*=\s*)?([A-Za-z_]\w*)(?:\.(?:geturl|url)(?:\(\))?)?\s*$/u.exec(destination)?.[1];
    if (identifier === undefined)
        return false;
    const escaped = identifier.replaceAll(/[$]/g, "\\$");
    const before = source.slice(0, sinkOffset);
    const parsed = new RegExp(`(?:${escaped}\\s*=\\s*)?(?:urlparse|urlsplit)\\s*\\(`).test(before);
    const exactHostGuard = new RegExp(`if\\s+${escaped}\\.(?:hostname|netloc)\\s+not\\s+in\\s+(?:ALLOWED_REDIRECT_HOSTS|allowed_redirect_hosts)[^\\n]*:[^\\n]*(?:return|raise)`).test(before);
    const exactOriginGuard = new RegExp(`if\\s+${escaped}\\.(?:hostname|netloc)\\s*!=\\s*(?:["'][^"']+["']|APP_HOST|app_host)[^\\n]*:[^\\n]*(?:return|raise)`).test(before);
    const schemeGuard = new RegExp(`if\\s+${escaped}\\.scheme\\s*(?:!=|not\\s+in)\\s*(?:["']https?["']|\\([^)]*["']https?["'])[^\\n]*:[^\\n]*(?:return|raise)`).test(before);
...
function analyzePythonFile(file, parsed, metrics, unknowns) {
    ...
    for (const range of executableRanges.filter((item) => item.name === "FunctionDefinition")) {
        ...
        const decorator = /@(?:app|router)\.(get|post|put|patch|delete|route)\s*\([^\n]*\)\s*(?:@[^\n]+\s*)*$/u.exec(prefix);
        if (decorator !== null) {
    ...
    const facts = new Map();
    const candidates = [];
    const applyPythonHtmlControl = (text, offset, fact) => {
        const htmlEscape = ((imports.has("markupsafe") || imports.has("flask")) && /\bescape\s*\(/u.test(text)) ||
            (imports.has("html") && /\bhtml\.escape\s*\(/u.test(text));
        const htmlSanitize = imports.has("bleach") && /\bbleach\.clean\s*\(/u.test(text);
        if (!htmlEscape && !htmlSanitize)
            return fact;
    ...
    const applyPythonUnknownTransformation = (text, fact) => {
        const recognizedHtmlCall = /\bescape\s*\(/u.test(text) || /\b(?:html\.escape|bleach\.clean)\s*\(/u.test(text);
        const unknownCall = !recognizedHtmlCall && /(?<!\.)\b(?!str\b|quote\b|urlencode\b)[A-Za-z_]\w*\s*\(/u.test(text);
    ...
    const expressionFact = (text, offset, route) => {
        ...
        if (flaskSource !== null && imports.has("flask")) {
            return applyPythonUnknownTransformation(text, applyPythonHtmlControl(text, offset, {
        ...
        if (route !== undefined) {
            for (const parameter of route.parameters) {
                if (parameter !== "self" && new RegExp(`\\b${parameter}\\b`).test(text)) {
                    return applyPythonUnknownTransformation(text, applyPythonHtmlControl(text, offset, facts.get(`${route.from}:${parameter}`) ?? {
    ...
    const addPythonCandidate = (kind, range, fact, sink, message, invariant, controlEvaluation = "ABSENT") => {
        ...
        candidates.push({
            ...
            proof: {
                ...
                securityControlEncountered: controlEvaluation !== "ABSENT" || (fact.controls?.length ?? 0) > 0,
                securityControlEvaluation: controlEvaluation,
                reachability: "likely",
                invariant,
                conclusion: `${invariant} is violated by a complete, reachable source-to-sink path.`,
    ...
    for (const range of executableRanges.filter((item) => item.name === "CallExpression")) {
        ...
        const pythonXss = ((imports.has("flask") || imports.has("markupsafe")) &&
        ...
        const pythonRedirect = (imports.has("flask") && /\b(?:flask\.)?redirect\s*\(/u.test(callText)) ||
            ((imports.has("starlette") || imports.has("fastapi")) &&
            ...
            : commandApi
                ...
                : filesystem
                    ...
                    : http
                        ...
                        : pythonXss
                            ? extractPythonCallArguments(callText, /\b(?:Markup|Response|HTMLResponse|make_response|render_template_string|markupsafe\.Markup)\s*\(/)
                            : pythonRedirect
                                ? extractPythonCallArguments(callText, /\b(?:flask\.)?(?:redirect|RedirectResponse)\s*\(/)
                                : undefined) ?? "";
        const command = osSystem || (subprocessApi && /\bshell\s*=\s*True\b/.test(argumentText));
        if (!database && !command && !filesystem && !http && !pythonXss && !pythonRedirect)
            continue;
        const firstArgument = (splitPythonTopLevelArguments(argumentText)[0] ?? "").replace(/^\s*(?:url|response)\s*=\s*/u, "");
        ...
        if (pythonXss) {
            ...
            if (!effective) {
                if (fact.unknownControl)
                    unknown("AS-XSS-001", "Python raw HTML rendering is reached through a transformation with UNKNOWN context-specific safety.");
                else
                    addPythonCandidate("XSS", range, fact, `Python raw HTML rendering (${context})`, `Proven untrusted input reaches a supported ${context} Python HTML rendering sink without a context-appropriate control.`, "UNTRUSTED_BROWSER_CONTENT_MUST_NOT_REACH_RAW_RENDERING_WITHOUT_A_CONTEXT_APPROPRIATE_CONTROL", controls.length > 0 ? "RECOGNIZED_INEFFECTIVE" : "ABSENT");
            }
        }
        if (pythonRedirect) {
            if (pythonRedirectPolicyEnforced(file.text, firstArgument, range.from)) {
                ...
                const weakPrefix = identifier !== undefined &&
                    new RegExp(`${identifier}\\.startswith\\s*\\(\\s*["']/["']\\s*\\)`).test(file.text.slice(route.from, range.from));
                const proofFact = weakPrefix
                    ? {
                        ...fact,
                        steps: [
    ...
    const flaskCsrfGlobal = imports.has("flask_wtf") && /\bCSRFProtect\s*\(\s*(?:app|application)\s*\)/u.test(file.text);
    ...
    for (const route of routeFunctions) {
        ...
        const mutatesState = /\bsession\s*\[[^\]]+\]\s*=(?!=)|\bsession\.(?:pop|clear)\s*\(/u.test(body) ||
        ...
        if (!mutatesState)
            continue;
        const flaskLogin = imports.has("flask_login") && /@login_required(?:\(\))?\s*$/mu.test(preamble);
        const flaskSession = imports.has("flask") &&
            /\bsession\s*(?:\[\s*["'](?:user|user_id|account|account_id|identity)["']\s*\]|\.get\s*\(\s*["'](?:user|user_id|account|account_id|identity)["'])/iu.test(body);
        const bearerOnly = /\b(?:HTTPBearer|OAuth2PasswordBearer|Authorization)\b/u.test(`${preamble}\n${body}`) &&
            /\b(?:jwt\.decode|jwt\.verify|decode_token|verify_token)\s*\(/u.test(body) &&
            !flaskSession;
        if (bearerOnly)
            continue;
        const csrfControl = flaskCsrfGlobal ||
            (imports.has("flask_wtf") && /\bvalidate_csrf\s*\(/u.test(body)) ||
            (/\b(?:request\.)?headers(?:\.get|\[)\s*\(?\s*["']Origin["']/iu.test(body) &&
                /(?:==|!=)\s*(?:["']https?:\/\/[^"']+["']|APP_ORIGIN|app_origin)|(?:ALLOWED_ORIGINS|allowed_origins)/u.test(body));
        if (csrfControl)
            continue;
        ...
        addPythonCandidate("CSRF", { from: route.from, to: route.to }, {
            sourceOffset: route.from,
...
function analyzeExpressCsrf(files, parsedByPath, securityIr, metrics, unknowns) {
    ...
    const packageScope = (filePath) => packageRoots.find((root) => root === "" || filePath === root || filePath.startsWith(`${root}/`)) ?? "";
    ...
    const importsAny = (path, packages) => [...(importsByPath.get(path)?.values() ?? [])].some((binding) => packages.has(binding.module));
    const expressSessionScopes = new Set();
    const sameSiteMiddleware = [];
    const globalAmbientMiddleware = [];
    for (const file of files) {
        const parsed = parsedByPath.get(file.relativePath);
        if (parsed?.language !== "javascript" && parsed?.language !== "typescript")
            continue;
        const imports = importsByPath.get(file.relativePath) ?? new Map();
        traverse(parsed.ast, {
            CallExpression(path_) {
                const chain = memberChain(path_.node.callee);
                if (chain?.properties.at(-1) !== "use")
                    return;
                for (const argument of path_.node.arguments) {
                    ...
                    if (argument.type !== "ArrowFunctionExpression" && argument.type !== "FunctionExpression")
                        continue;
                    const request = argument.params[0]?.type === "Identifier" ? argument.params[0].name : undefined;
                    const response = argument.params[1]?.type === "Identifier" ? argument.params[1].name : undefined;
                    const next = argument.params[2]?.type === "Identifier" ? argument.params[2].name : undefined;
                    if (request === undefined || response === undefined || next === undefined)
                        continue;
                    ...
                    const guardedSessionIdentity = new RegExp(`if\\s*\\(\\s*!\\s*${escapedRequest}(?:\\?\\.)?\\.session(?:\\?\\.)?\\.(?:user|userId|account|accountId|auth|authenticated|loggedin|passport|identity|subject)\\s*\\)[\\s\\S]{0,500}(?:return\\s+)?${escapedResponse}(?:\\?\\.)?\\.(?:redirect|status|sendStatus|end)\\s*\\(`, "u").test(callbackText);
    ...
    const ambientEvidence = (symbolIds) => {
        for (const symbolId of symbolIds) {
            ...
            const session = new RegExp(`\\b${escaped}(?:\\?\\.)?\\.session(?:\\?\\.)?\\.(?:user|userId|account|accountId|auth|authenticated|loggedin|passport|identity|subject)\\b(?!\\s*=(?!=))`, "u").test(source.text);
            const cookie = new RegExp(`\\b${escaped}(?:\\?\\.)?\\.cookies(?:\\?\\.)?\\.(?:session|sessionId|sid|auth|authToken)\\b`, "iu").test(source.text);
    ...
    const inlineRouteHandler = (route) => {
        ...
        traverse(parsed.ast, {
            CallExpression(path_) {
                if (path_.node.start !== route.location.start.offset)
                    return;
                const handler = path_.node.arguments.at(-1);
                if (handler?.type !== "ArrowFunctionExpression" && handler?.type !== "FunctionExpression")
                    return;
                const requestName = handler.params[0]?.type === "Identifier" ? handler.params[0].name : "req";
                result = {
    ...
    const inlineAmbientEvidence = (handler) => {
        if (handler === undefined)
            return undefined;
        const escaped = handler.requestName.replaceAll(/[$]/g, "\\$");
        const session = new RegExp(`\\b${escaped}(?:\\?\\.)?\\.session(?:\\?\\.)?\\.(?:user|userId|account|accountId|auth|authenticated|loggedin|passport|identity|subject)\\b(?!\\s*=(?!=))`, "u").test(handler.text);
        const cookie = new RegExp(`\\b${escaped}(?:\\?\\.)?\\.cookies(?:\\?\\.)?\\.(?:session|sessionId|sid|auth|authToken)\\b`, "iu").test(handler.text);
        return (session && expressSessionScopes.has(packageScope(handler.file.relativePath))) || cookie
            ? handler
            : undefined;
    };
    const inlineEstablishesAmbientSession = (handler) => {
        if (handler === undefined)
            return false;
        const escaped = handler.requestName.replaceAll(/[$]/g, "\\$");
        return new RegExp(`\\b${escaped}(?:\\?\\.)?\\.session(?:\\?\\.)?\\.(?:user|userId|account|accountId|auth|authenticated|loggedin|passport|identity|subject)\\b\\s*=(?!=)`, "u").test(handler.text);
    };
    const nonAmbientAuth = (symbolIds) => symbolIds.some((symbolId) => {
        const source = symbolText(symbolId);
        if (source === undefined)
            return false;
        return (importsAny(source.file.relativePath, new Set(["jsonwebtoken", "jose"])) &&
            /\b(?:headers(?:\?\.)?\.authorization|get\s*\(\s*["']authorization["'])/iu.test(source.text) &&
            /\b(?:verify|jwtVerify)\s*\(/u.test(source.text));
    });
    const handlerControl = (symbolIds) => symbolIds.some((symbolId) => {
        const source = symbolText(symbolId);
        if (source === undefined)
            return false;
        const csrfToken = importsAny(source.file.relativePath, CSRF_PACKAGES) &&
            /\b(?:verifyToken|validateRequest|doubleCsrfProtection|csrfProtection)\s*\(/u.test(source.text);
        const exactOrigin = /new\s+URL\s*\([^)]*(?:headers(?:\?\.)?\.origin|get\s*\(\s*["']origin["'])[^)]*\)\.origin/iu.test(source.text) &&
            /(?:===|!==)\s*(?:["']https?:\/\/[^"']+["']|APP_ORIGIN|appOrigin)|(?:ALLOWED_ORIGINS|allowedOrigins)\.has\s*\(/u.test(source.text);
        return csrfToken || exactOrigin;
    });
    const inlineNonAmbientAuth = (handler) => handler !== undefined &&
    ...
    const inlineHandlerControl = (handler) => handler !== undefined &&
        ((importsAny(handler.file.relativePath, CSRF_PACKAGES) &&
            /\b(?:verifyToken|validateRequest|doubleCsrfProtection|csrfProtection)\s*\(/u.test(handler.text)) ||
            (/new\s+URL\s*\([^)]*(?:headers(?:\?\.)?\.origin|get\s*\(\s*["']origin["'])[^)]*\)\.origin/iu.test(handler.text) &&
                /(?:===|!==)\s*(?:["']https?:\/\/[^"']+["']|APP_ORIGIN|appOrigin)|(?:ALLOWED_ORIGINS|allowedOrigins)\.has\s*\(/u.test(handler.text)));
    const sourceMutatesState = (file, text, requestName) => {
        const escaped = requestName.replaceAll(/[$]/g, "\\$");
        const sessionMutation = new RegExp(`\\b${escaped}(?:\\?\\.)?\\.session(?:\\?\\.)?(?:\\.[A-Za-z_$][\\w$]*|\\[[^\\]]+\\])\\s*=(?!=)|\\b${escaped}(?:\\?\\.)?\\.session(?:\\?\\.)?\\.(?:destroy|regenerate)\\s*\\(`, "u").test(text);
        const processMutation = importsAny(file.relativePath, new Set(["node:child_process", "child_process"])) &&
            /\b(?:exec|execSync|spawn|spawnSync)\s*\(/u.test(text);
        const filesystemMutation = importsAny(file.relativePath, new Set(["node:fs", "fs"])) &&
            /\b(?:writeFile|writeFileSync|appendFile|appendFileSync|unlink|unlinkSync|rename|renameSync)\s*\(/u.test(text);
        return sessionMutation || processMutation || filesystemMutation;
    };
    const handlerMutatesState = (symbolIds, inlineHandler) => {
        for (const symbolId of symbolIds) {
            const source = symbolText(symbolId);
            const symbol = symbolsById.get(symbolId);
            if (source !== undefined &&
        ...
        return (inlineHandler !== undefined &&
            sourceMutatesState(inlineHandler.file, inlineHandler.text, inlineHandler.requestName));
    };
    const routeMiddlewareState = (route) => {
        const file = filesByPath.get(route.location.path);
        const parsed = parsedByPath.get(route.location.path);
        if (file === undefined ||
            (parsed?.language !== "javascript" && parsed?.language !== "typescript"))
            return { hasCsrf: false, unresolved: true, receiver: undefined };
        const imports = importsByPath.get(file.relativePath) ?? new Map();
        const resolvedNames = new Set(route.middlewareSymbolIds
            .map((id) => symbolsById.get(id)?.name)
            .filter((name) => name !== undefined));
        let result = { hasCsrf: false, unresolved: false, receiver: undefined };
        let routeReceiver;
        const globalCsrfReceivers = new Set();
        const isCsrfArgument = (path_, argument) => {
            if (argument.type !== "Identifier")
                return false;
            ...
            if (binding?.isVariableDeclarator() && binding.node.init?.type === "CallExpression") {
                ...
                return (callee.type === "Identifier" && CSRF_PACKAGES.has(imports.get(callee.name)?.module ?? ""));
            }
            return false;
        };
        traverse(parsed.ast, {
            CallExpression(path_) {
                const chain = memberChain(path_.node.callee);
                if ((path_.node.start ?? Number.MAX_SAFE_INTEGER) < route.location.start.offset &&
                ...
                for (const argument of middleware) {
                    ...
                    if (!resolvedNames.has(argument.name))
                        result = { ...result, unresolved: true };
                }
            },
        });
    ...
    for (const route of securityIr.routes) {
        ...
        if (route.handlerSymbolId === undefined && inlineHandler === undefined) {
            unknowns.push({
                ...
                reasonCodes: ["INSUFFICIENT_DATAFLOW_PROOF"],
                explanation: "A state-changing Express route has an unresolved handler, so authentication and browser reachability are UNKNOWN.",
            });
            continue;
        }
        const symbolIds = [
        ...
        if (!handlerMutatesState(symbolIds, inlineHandler))
            continue;
        ...
        if (ambient === undefined && middleware.receiver !== undefined) {
            ambient = globalAmbientMiddleware.find((candidate) => candidate.file.relativePath === route.location.path &&
        ...
        if (ambient === undefined && inlineEstablishesAmbientSession(inlineHandler))
            continue;
        ...
        if (middleware.hasCsrf || handlerControl(symbolIds) || inlineHandlerControl(inlineHandler))
            continue;
        const sameSiteOnly = middleware.receiver !== undefined &&
            sameSiteMiddleware.some((candidate) => candidate.filePath === route.location.path &&
                candidate.receiver === middleware.receiver &&
                candidate.offset < route.location.start.offset);
        const customResolvedMiddleware = route.middlewareSymbolIds.some((id) => ambientEvidence([id]) === undefined && !handlerControl([id]) && !nonAmbientAuth([id]));
        if (ambient === undefined ||
            middleware.unresolved ||
            customResolvedMiddleware ||
            sameSiteOnly) {
            const reasons = ["INSUFFICIENT_DATAFLOW_PROOF"];
            if (middleware.unresolved)
                reasons.push("UNSUPPORTED_FRAMEWORK_PATTERN");
            unknowns.push({
                ruleId: "AS-CSRF-001",
                path: route.location.path,
                line: route.location.start.line,
                reasonCodes: [...new Set(reasons)],
                explanation: ambient === undefined
        ...
        candidates.push({
            ...
            proof: {
                ...
                source: {
                    kind: "SOURCE",
                    label: "Supported Express session/cookie authentication supplies an ambient browser credential",
                    location: {
                ...
                reachability: "likely",
                invariant: "AMBIENTLY_AUTHENTICATED_STATE_CHANGING_BROWSER_REQUESTS_MUST_PROVE_REQUEST_ORIGIN",
                conclusion: "The supported state-changing route is reachable with ambient credentials and no supported request-origin control is bound to it.",
                proofState: "PROVEN_INSECURE",
                analysisLimitations: [
...
export function analyzeApplicationDataflow(files, parsedByPath, securityIr) {
    const metrics = {
        filesAnalyzed: 0,
        astNodesVisited: 0,
        factsCreated: 0,
