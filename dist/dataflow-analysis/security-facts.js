...
function classifyRequestExpression(expression, requestName, authenticated) {
...
export function enrichSecurityFacts(input, files, parsedByPath) {
    ...
    const routeIdsByHandler = new Map();
    for (const route of input.routes) {
        if (route.handlerSymbolId === undefined)
            continue;
        const routes = routeIdsByHandler.get(route.handlerSymbolId) ?? new Set();
        routes.add(route.id);
        routeIdsByHandler.set(route.handlerSymbolId, routes);
    }
    const provenRouteIdsByHandler = new Map();
    for (const proof of authProofs) {
        const routes = provenRouteIdsByHandler.get(proof.handlerSymbolId) ?? new Set();
        routes.add(proof.routeId);
        provenRouteIdsByHandler.set(proof.handlerSymbolId, routes);
    }
    const authenticatedHandlers = new Map();
    for (const proof of authProofs) {
    ...
    let calls = input.calls.map((call) => ({
        ...call,
    ...
    for (let iteration = 0; iteration < Math.max(1, input.symbols.length); iteration += 1) {
        ...
        for (const call of calls) {
            ...
            for (const argument of call.arguments) {
                ...
                recordIdentity(callee.id, parameterName, { source: "derived", trust: combineTrust(sources) }, call.location, `${parameterName} derives from call argument ${argument.position} supplied by ${symbolsById.get(call.callerSymbolId)?.name ?? "caller"}.`, sources.map((fact) => fact.id).sort());
...
//# sourceMappingURL=security-facts.js.map