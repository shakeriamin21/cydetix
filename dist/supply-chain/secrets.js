...
const PATTERNS = [
    ...
    },
    {
        ...
        expression: /\b(?:AWS_SECRET_ACCESS_KEY|DATABASE_PASSWORD|CLIENT_SECRET|PRIVATE_API_KEY|API_TOKEN|ACCESS_TOKEN)\b\s*(?:=|:)\s*["']([^"'\r\n]{24,})["']/giu,
        valueGroup: 1,
        contextual: true,
    },
    {
...
function looksLikePlaceholder(value) {
    ...
    if (compact.length > 0 && new Set(compact.toLowerCase()).size <= 2)
        return true;
    if (/^[a-f0-9]{32,128}$/iu.test(value))
        return true;
    if (/^[0-9a-f]{8}-[0-9a-f-]{27,}$/iu.test(value))
        return true;
    return false;
}
function redacted(provider, value) {
    return `[REDACTED ${provider} credential; length=${value.length}; sha256=${sha256(value).slice(0, 12)}]`;
}
export function detectSecretsInText(text, options) {
    ...
    for (const pattern of PATTERNS) {
        for (const match of text.matchAll(pattern.expression)) {
            ...
            results.push(secretExposureSchema.parse({
...
export function analyzeWorkingTreeSecrets(files) {
    const exposures = files.flatMap((file) => detectSecretsInText(file.text, {
        path: file.relativePath,
        sourceCategory: "working-tree",
        historyState: "current",
    }));
    const unique = [
        ...new Map(exposures.map((exposure) => [`${exposure.location.path}:${exposure.fingerprint}`, exposure])).values(),
    ];
