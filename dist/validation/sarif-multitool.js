...
import { access as accessFile, chmod as changeMode, readFile, realpath as resolveRealPath, stat, } from "node:fs/promises";
...
function expectedPlatformPackage(platform) {
    return {
        name: `@microsoft/sarif-multitool-${platform}`,
        executable: platform === "win32" ? "Sarif.Multitool.exe" : "Sarif.Multitool",
    };
}
export async function prepareTrustedSarifMultitoolExecutable(installation, fileSystem = defaultFileSystem) {
    ...
    try {
        ...
        const expectedExecutable = await fileSystem.realpath(path.join(path.dirname(platformPackageJson), expected.executable));
        ...
        await fileSystem.access(executable, platform === "win32" ? fileSystemConstants.F_OK : fileSystemConstants.X_OK);
    }
    catch {
...
export async function resolveSarifMultitoolExecutable() {
    ...
    return prepareTrustedSarifMultitoolExecutable({
        ...
        platformPackageJsonPath: packageRequire.resolve(`@microsoft/sarif-multitool-${platform}/package.json`),
    });
}
//# sourceMappingURL=sarif-multitool.js.map