...
import { z } from "zod";
export const sourceBoundEvidenceFormatVersion = "1.0.0";
...
const relativePathSchema = z
    ...
    !value.startsWith("/") &&
    !/^[A-Za-z]:/u.test(value) &&
    !value.split("/").some((segment) => segment === "" || segment === "." || segment === ".."), "Evidence paths must be canonical repository-relative paths.");
...
export function validateSourceBoundEvidenceSet(indexInput, blobs, expected) {
    ...
    for (const blob of blobs) {
        if (blobsByPath.has(blob.path))
            throw new Error(`Duplicate evidence blob ${blob.path}.`);
...
export function requireReadyEvidence(evidence, requiredTypes) {
    for (const type of requiredTypes) {
        const record = evidence.get(type);
        if (record === undefined)
            throw new Error(`Missing required evidence ${type}.`);
        if (record.result !== "PASS")
            throw new Error(`Required evidence ${type} is ${record.result}, not PASS.`);
    }
}
//# sourceMappingURL=source-bound-evidence.js.map