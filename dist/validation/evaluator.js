import { corpusLabelsSchema, corpusManifestSchema, corpusValidationResultSchema, } from "./model.js";
...
function metrics(counts, completeness) {
    const limitations = [];
    ...
    if (completeness !== "COMPLETE_APPLICABLE_LABELS")
        limitations.push("Recall, false-positive rate, specificity, F1, and Youden are withheld for incomplete labels.");
    if (completeness !== "COMPLETE_APPLICABLE_LABELS")
    ...
    const youden = recall === undefined || specificity === undefined
        ? undefined
        : {
            value: recall.value + specificity.value - 1,
            numerator: recall.numerator * specificity.denominator +
...
function increment(counts, result) {
    counts.total += 1;
    const mapping = {
    ...
    if (field !== undefined)
        counts[field] += 1;
    if (result.languageCompatible)
        counts.languageCompatible += 1;
    if (result.ruleCompatible)
        counts.ruleCompatible += 1;
    if (["TRUE_POSITIVE", "FALSE_POSITIVE", "TRUE_NEGATIVE", "FALSE_NEGATIVE"].includes(result.disposition))
        counts.executed += 1;
}
function matches(finding, ruleId, expectedPath, expectedLine) {
...
export function evaluateCorpus(manifestInput, labelsInput, findings) {
    ...
    for (const item of labels.expectedCases) {
        const observed = findings.some((finding) => matches(finding, item.ruleId, item.path, item.line));
    ...
    for (const finding of findings) {
        ...
        if (!represented)
            cases.push({
                ...
                ruleCompatible: true,
                disposition: "UNKNOWN",
                rationale: "Observed scanner finding has no independent expected label or manual adjudication.",
            });
    }
    cases.sort((left, right) => `${left.ruleId}:${left.path}:${left.caseId}`.localeCompare(`${right.ruleId}:${right.path}:${right.caseId}`));
