...
function createDockerCommand(dockerExecutable, environment) {
...
function inspectContainer(containerName, dockerCommand) {
    const inspection = dockerCommand(["container", "inspect", containerName, "--format", "{{.State.Status}}"], 5_000);
    ...
    const listing = dockerCommand(["ps", "-a", "--no-trunc", "--filter", `name=^/${containerName}$`, "--quiet"], 5_000);
    ...
    return {
        state: "UNKNOWN",
        failure: `container inspection failed (inspect=${String(inspection.error?.message ?? inspection.status)}; list=${String(listing.error?.message ?? listing.status)})`,
    };
}
export function removeAndConfirmContainer(containerName, dockerCommand, options = {}) {
    ...
    if (initial.state === "PRESENT")
        observedStates.push(initial.containerState);
    else if (initial.state === "UNKNOWN")
...
function assertContainerCleanup(result, context) {
    if (result.state === "FAILED")
        throw new Error(`${context} cleanup failed after ${result.attempts} attempts: ${result.failure ?? "absence was not confirmed"}.`);
}
...
export function hardenedContainerProfileFailures(inspected, expectedWorkingDirectory = "/workspace") {
    ...
    const inspectCapability = async () => {
        ...
        try {
            ...
            const probe = profileReceived
                ...
                : undefined;
            const probeCleanup = removeAndConfirmContainer(probeContainerName, createDockerCommand(docker, environment), probeCreationMayBeInFlight
                ? { maximumAttempts: 120, retryMilliseconds: 250, absenceConfirmations: 120 }
                : undefined);
            if (probeCleanup.state !== "FAILED")
                probeContainerName = undefined;
            if (!profileReceived ||
                probe === undefined ||
                probe.spawnFailed ||
                probe.timedOut ||
            ...
            if (probeContainerName !== undefined) {
                const environment = dockerEnvironment(configurationDirectory);
                const finalProbeCleanup = removeAndConfirmContainer(probeContainerName, createDockerCommand(docker, environment), probeCreationMayBeInFlight
                    ? { maximumAttempts: 120, retryMilliseconds: 250, absenceConfirmations: 120 }
                    : undefined);
            ...
            try {
                ...
                if (creation.error !== undefined || creation.status !== 0) {
                    result = verificationExecutionResultSchema.parse({
                        ...
                        outputTruncated: Buffer.byteLength(creation.stdout) + Buffer.byteLength(creation.stderr) >
                            MAX_OUTPUT_BYTES,
                        capability,
                        message: "Container creation failed within a bounded Docker invocation; no local fallback was attempted.",
                    });
                }
                else {
                    creationMayBeInFlight = false;
                    const inspection = dockerCommand(["container", "inspect", activeContainerName, "--format", "{{json .}}"], 10_000);
                    let inspected;
                    try {
                        inspected = JSON.parse(inspection.stdout);
                    }
                    ...
                    const profileFailures = [
                        ...(inspection.error === undefined && inspection.status === 0
                            ? []
                            : ["container inspection command"]),
                        ...hardenedContainerProfileFailures(inspected, path.posix.resolve("/workspace", command.workingDirectory.replaceAll("\\", "/"))),
                    ];
                    if (profileFailures.length > 0) {
                        ...
                        const execution = await runBoundedProcess(docker, ["start", "--attach", activeContainerName], environment, command.timeoutMilliseconds, () => {
                ...
                if (containerName !== undefined && configurationDirectory !== undefined) {
                    cleanupResult = removeAndConfirmContainer(containerName, createDockerCommand(docker, dockerEnvironment(configurationDirectory)), creationMayBeInFlight
                        ? { maximumAttempts: 120, retryMilliseconds: 250, absenceConfirmations: 120 }
                        : undefined);
                }
