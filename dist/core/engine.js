...
function applyAcceptedRisk(findings, config, now) {
    const active = [];
    const suppressed = [];
    ...
    for (const finding of findings) {
        ...
        const exception = config.suppressions.find((candidate) => candidate.rule === finding.ruleId &&
...
function summarize(findings, suppressed) {
...
export async function scanRepository(options) {
    ...
    for (const file of traversal.files) {
        ...
        if (isParseFailure(result))
            ...
            parsedByPath.set(file.relativePath, result);
    }
    ...
    const applicationDataflowResult = analyzeApplicationDataflow(traversal.files, parsedByPath, applicationSecurityIr);
    ...
    const authenticationAnalysis = buildAuthenticationAnalysis(securityIr, manifest, traversal.files, parsedByPath);
    ...
    for (const file of traversal.files) {
        ...
        for (const rule of SECURITY_RULES)
            rawFindings.push(...rule.analyze(context));
    }
    ...
    const unsupportedFrameworks = manifest.frameworks.filter((framework) => !supportedFrameworks.has(framework));
    const authenticationGraphStart = performance.now();
    const authGraph = buildAuthenticationGraph(manifest, traversal.files, securityIr, authorizationProofs, authenticationAnalysis);
    ...
    const report = {
        ...
        coverage: {
            tier: "phase-four",
            analyzedLanguages: manifest.languages.filter((language) => ["javascript", "typescript", "python"].includes(language)),
            analyzedFrameworks: manifest.frameworks.filter((framework) => supportedFrameworks.has(framework)),
            enginesRun: [
            ...
            analysisCompleteness: [
                ...
                },
                {
                    ...
                    details: parseFailures.length === 0
        ...
        reproducibility: {
            ...
            enabledRules: RULES.map((rule) => ({ id: rule.id, version: rule.version })).sort((left, right) => left.id.localeCompare(right.id)),
            ...
            suppressionFingerprint: stableObjectFingerprint({
                ...
                suppressions: [...config.suppressions].sort((left, right) => `${left.rule}:${left.scope}:${left.fingerprint ?? ""}`.localeCompare(`${right.rule}:${right.scope}:${right.fingerprint ?? ""}`)),
            }),
