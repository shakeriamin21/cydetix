...
import { SECURITY_CONTROLS, securityControlRegistryFingerprint, } from "../security-controls/registry.js";
...
export function createTrustReport() {
...
export function renderRulesText(report = createRulesReport()) {
    const lines = [
    ...
    for (const rule of report.rules) {
        lines.push(`${rule.id}@${rule.version} [${rule.maturity}] ${rule.title}`, `  ${rule.category}; ${rule.cwe.join(", ")}; OWASP ${rule.owaspTop10.join(", ")}; ASVS ${rule.asvs.join(", ") || "not mapped"}; ceiling ${rule.maxRemediationClass}`);
    }
    return `${lines.join("\n")}\n`;
}
export function renderTrustText(report = createTrustReport()) {
    return [
