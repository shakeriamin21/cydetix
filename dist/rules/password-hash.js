...
function analyzeJavaScript(context) {
    ...
    traverse(context.parsed.ast, {
        CallExpression(callPath) {
            if (memberName(callPath.node.callee) !== "createHash")
                return;
            const algorithm = literalString(callPath.node.arguments[0])?.toLowerCase();
            if (algorithm === undefined || !FAST_HASHES.has(algorithm))
                return;
            const container = callPath.findParent((candidate) => candidate.isStatement() || candidate.isVariableDeclarator());
            const node = container?.node ?? callPath.node;
            const range = nodeRange(node);
            const hashRange = nodeRange(callPath.node);
            if (range === undefined || hashRange === undefined)
                return;
            const source = context.file.text.slice(range.start, range.end);
            if (!PASSWORD_CONTEXT.test(source))
                return;
...
function analyzePython(context) {
    if (context.parsed?.language !== "python")
        return [];
    const findings = [];
    const pattern = /hashlib\.(md5|sha1|sha256|sha512)\s*\(([^\n)]*(?:password|passwd|pwd|passphrase|credential)[^\n)]*)\)/gi;
