...
function invariantRule(config) {
    const definition = requireRule(config.ruleId);
    return {
        definition,
        analyze(context) {
            if (context.authenticationAnalysis === undefined)
                return [];
            const filesByPath = new Map(context.files.map((file) => [file.relativePath, file]));
            const operationsById = new Map(context.authenticationAnalysis.operations.map((operation) => [operation.id, operation]));
            return context.authenticationAnalysis.results.flatMap((result) => {
                if (result.invariantId !== config.invariantId ||
                ...
                const anchor = config.anchorKinds
...
//# sourceMappingURL=authentication-invariants.js.map