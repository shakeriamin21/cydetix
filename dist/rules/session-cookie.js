...
import { asObject, literalString, memberName, objectProperty, propertyExpression, } from "../ast-analysis/babel-utils.js";
...
function reportFlag(context, flag, value) {
    if (value.value)
        return undefined;
    ...
    return makeFinding({
        ...
        ...(isSafe
            ? {
                fix: makeFix(context.file, range.start, range.end, "true", "Enable HttpOnly for the session cookie."),
            }
            : {}),
        fingerprintAnchor: `${flag}:${context.file.text.slice(Math.max(0, range.start - 80), range.end + 80)}`,
    });
}
function inspectCookieOptions(context, options) {
    const findings = [];
...
function expressCookieOptions(call) {
    if (memberName(call.callee) !== "cookie")
        return undefined;
    const cookieName = literalString(call.arguments[0]);
    if (cookieName === undefined || !SESSION_NAME.test(cookieName))
        return undefined;
    return asObject(call.arguments[2]);
}
function expressSessionOptions(context, call) {
    if (!context.file.text.includes("express-session"))
        return undefined;
    ...
    return asObject(propertyExpression(options === undefined ? undefined : objectProperty(options, "cookie")));
}
function analyzeJavaScript(context) {
    ...
    traverse(context.parsed.ast, {
        CallExpression(callPath) {
            const options = expressCookieOptions(callPath.node) ?? expressSessionOptions(context, callPath.node);
            if (options !== undefined)
                findings.push(...inspectCookieOptions(context, options));
        },
    });
    return findings;
}
function analyzePython(context) {
    ...
    for (const match of context.file.text.matchAll(pattern)) {
        ...
        if (flagName === undefined || falseText === undefined)
            continue;
        ...
        findings.push(makeFinding({
            ...
            ...(isSafe
                ? {
                    fix: makeFix(context.file, startOffset, startOffset + falseText.length, "True", "Enable HttpOnly for the session cookie."),
                }
                : {}),
