...
export function buildSupplyChainFindings(result, files) {
    const findings = [];
    const analysis = result.analysis;
    const evidenceById = new Map(analysis.ir.evidence.map((item) => [item.id, item]));
    const packageByPurl = new Map(analysis.inventory.packages.map((component) => [component.purl, component]));
    for (const advisory of analysis.advisories.advisories) {
        ...
        const file = packageEvidence === undefined ? undefined : fileByPath(files, packageEvidence.location.path);
        if (file === undefined || packageEvidence === undefined)
            continue;
        ...
        const fixed = advisory.fixedVersions.length === 0 ? "not supplied" : advisory.fixedVersions.join(", ");
        const evidencePath = [
            ...dependencyPath.map((entry, index) => ({
                ...
                message: index === 0 ? `dependency root ${entry}` : `resolved dependency path includes ${entry}`,
            })),
            {
    ...
    for (const reference of analysis.ci.actionReferences) {
        if (["full-sha", "local", "digest"].includes(reference.pinning))
            continue;
        const file = fileByPath(files, reference.workflow);
        if (file === undefined)
            continue;
        findings.push(workflowFinding(actionPinning, file, reference.location.start.offset, `${reference.repository}@${reference.reference}`, `${reference.repository} uses ${reference.pinning} reference ${reference.reference}; no authoritative SHA was invented.`));
    }
    for (const permission of analysis.ci.permissions) {
        if (permission.access !== "write-all")
            continue;
        const file = fileByPath(files, permission.workflow);
        if (file === undefined)
            continue;
        findings.push(workflowFinding(broadPermissions, file, permission.location.start.offset, "permissions: write-all", `${permission.job === undefined ? "Workflow" : `Job ${permission.job}`} explicitly grants write-all permissions.`));
    }
    for (const signal of result.workflowSignals) {
        const rule = signal.kind === "dangerous-pull-request-target" ? dangerousPrTarget : contextInjection;
        ...
        findings.push(workflowFinding(rule, signal.workflow, first.offset, first.needle, signal.message, evidencePath.length >= 2 ? evidencePath : undefined));
    }
    findings.push(...analysis.secrets.exposures
