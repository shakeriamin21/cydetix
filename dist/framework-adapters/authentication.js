...
function importsFrom(declarations) {
...
function importedCall(call, imports) {
    ...
    if (parts.receiver === undefined || parts.property === undefined)
        return undefined;
...
function nestedMemberText(file, expression) {
    return sourceText(file, expression).replaceAll(/\s+/g, "");
}
function literalArgument(call, position) {
...
function classifyPrismaCall(call, callText, symbol, sourceLocation) {
            ...
            return [
                {
                    ...
                    attributes: {
                        checksExpiry: /expiresAt|expiry|expires/.test(callText) &&
...
function classifyImportedCall(call, imported, callText, symbol, sourceLocation) {
...
function classifySessionCall(call, file, symbol, sourceLocation, hasExpressSession) {
        ...
        return [
            {
                ...
                guarantee: "res.clearCookie removes the browser cookie but does not invalidate server-side session state.",
...
function classifyWeakRandomCall(call, symbol, sourceLocation) {
    if (call.callee.type !== "MemberExpression" ||
        call.callee.computed ||
...
function classifyProtocolParameter(call, symbol, sourceLocation) {
    if (call.callee.type !== "MemberExpression" || call.callee.computed)
        return [];
    if (call.callee.property.type !== "Identifier" || call.callee.property.name !== "set")
        return [];
    const parameter = literalArgument(call, 0);
    if (!["state", "code_challenge", "code_challenge_method", "nonce", "scope"].includes(parameter ?? "")) {
        return [];
    }
    const value = literalArgument(call, 1);
    const attributes = { parameter: parameter ?? "unknown" };
    if (parameter === "state")
        attributes.stateSent = "true";
    if (parameter === "code_challenge")
        attributes.challengeSent = "true";
    if (parameter === "code_challenge_method")
        attributes.method = value ?? "dynamic";
    if (parameter === "nonce")
        attributes.nonceSent = "true";
    if (parameter === "scope" && value?.split(/\s+/).includes("openid"))
        attributes.oidc = "true";
    return [
        {
            kind: "OAuthAuthorizationRequest",
...
function assignmentOperation(assignment, file, symbol, sourceLocation, hasExpressSession) {
    if (!hasExpressSession || assignment.left.type !== "MemberExpression")
        return [];
    ...
    if (!/\.session\.(?:userId|accountId|subjectId|principal|user)$/.test(left))
        return [];
    return [
        {
            ...base,
...
function tokenAcceptanceAssignment(assignment, file, symbol, sourceLocation, hasJwt) {
    if (!hasJwt || assignment.left.type !== "MemberExpression")
        return [];
    const left = nestedMemberText(file, assignment.left);
    if (!/\.(?:auth|user)$/.test(left))
        return [];
    return [
        {
            kind: "TokenAccept",
            ...
            guarantee: "JWT-derived data is accepted as application authenticated context at this assignment.",
            ...(symbol === undefined ? {} : { functionSymbolId: symbol.id }),
            location: sourceLocation,
            confidence: "high",
            attributes: { normalizedOperation: "TOKEN_ACCEPT" },
...
function sessionLookupMember(member, file, symbol, sourceLocation, hasExpressSession) {
    if (!hasExpressSession)
        return [];
    const text = nestedMemberText(file, member);
    if (!/\.session\.(?:userId|accountId|subjectId|principal|user)$/.test(text))
        return [];
    return [
        {
            kind: "SessionLookup",
...
export function analyzeAuthenticationOperations(ir, manifest, files, parsedByPath) {
    ...
    const publicOAuthClientFiles = new Set(files
        ...
        traverse(parsed.ast, {
            CallExpression(callPath) {
                ...
                for (const input of classifyPrismaCall(callPath.node, text, symbol, sourceLocation))
                    add(input);
                for (const input of classifyImportedCall(callPath.node, importedCall(callPath.node, imports), text, symbol, sourceLocation))
                    add(input);
                for (const input of classifySessionCall(callPath.node, file, symbol, sourceLocation, hasExpressSession))
                    add(input);
            ...
            MemberExpression(memberPath) {
                if (memberPath.parentPath.isAssignmentExpression() &&
