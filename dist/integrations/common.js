...
export async function updateJsonServer(boundary, filePath, rootKey, server, remove, dryRun) {
    ...
    if (remove)
        delete servers.cydetix;
    else
        servers.cydetix = server;
...
export function combineIntegrationStates(states) {
    if (states.includes("configuration_inaccessible"))
        return "configuration_inaccessible";
    if (states.includes("unsupported_version"))
        return "unsupported_version";
    if (states.every((state) => state === "configured"))
        return "configured";
    if (states.every((state) => state === "not_configured"))
        return "not_configured";
    return "partially_configured";
}
