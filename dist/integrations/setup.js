...
function displayStatus(detection) {
    if (!detection.detected && detection.id !== "generic-mcp")
...
export async function runSetup(options = {}) {
    ...
    if (options.status === true || options.verify === true) {
        ...
        for (const detection of installed)
            emit(options, `  ${detection.displayName} — ${displayStatus(detection)}\n`);
    if (options.status === true || options.verify === true) {
        const relevant = selected.length > 0 ? detections.filter((item) => selected.includes(item.id)) : visible;
        const verified = relevant.length > 0 &&
            relevant.every((detection) => !detection.detected || detection.integration === "configured");
        emit(options, options.verify
    ...
    for (const id of selected) {
        ...
        if (adapter !== undefined)
            emit(options, `  ${adapter.displayName}\n`);
    }
    const accepted = options.yes === true ||
        dryRun ||
        (await confirm(options.remove
            ? "Remove Cydetix from these AI agents? [Y/n] "
            : "Enable automatic Cydetix security checks in these AI agents? [Y/n] ", options));
    if (!accepted) {
        emit(options, interactive(options)
            ? "\nSetup cancelled.\n"
...
export async function runAutomaticIntegration(options = {}) {
    ...
    const accepted = await confirm("\nEnable automatic security checks in these AI agents? [Y/n] ", options);
    if (!accepted) {
    ...
    for (const result of report.results)
        process.stdout.write(`  ${result.displayName} ${result.verified ? "connected" : "needs attention"}\n`);
