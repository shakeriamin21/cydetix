...
import { commandAvailable, existingPaths, inspectYamlEntry, pinnedMcpServer, updateYamlEntry, } from "../common.js";
import { agentDetection } from "../discovery/index.js";
function configPath(context) {
    return context.platform === "win32"
        ? path.join(context.homeDirectory, "AppData", "Roaming", "Block", "goose", "config", "config.yaml")
...
async function integrationState(context) {
    return inspectYamlEntry(context.homeBoundary, configPath(context), "extensions", "cydetix", extension(context));
}
async function change(context, remove, dryRun) {
    const before = await integrationState(context);
    const config = configPath(context);
    const changed = await updateYamlEntry(context.homeBoundary, config, "extensions", "cydetix", extension(context), remove, dryRun);
    const after = dryRun
        ? remove
            ? "not_configured"
            : "configured"
