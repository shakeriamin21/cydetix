...
async function integrationState(context) {
    return inspectJsonServer(context.homeBoundary, configPath(context), "mcpServers", pinnedMcpServer(context));
}
async function change(context, remove, dryRun) {
    const before = await integrationState(context);
    const config = configPath(context);
    const changed = await updateJsonServer(context.homeBoundary, config, "mcpServers", pinnedMcpServer(context), remove, dryRun);
    const after = dryRun
        ? remove
            ? "not_configured"
            : "configured"
