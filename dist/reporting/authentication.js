...
export function renderAuthenticationText(report) {
    ...
    const applicableUnknown = analysis.results.filter((result) => result.applicability === "APPLICABLE" && result.conclusion === "UNKNOWN");
    const applicabilityUnknown = analysis.results.filter((result) => result.applicability === "UNKNOWN");
    const lines = [
    ...
    for (const protocol of protocols) {
        ...
        lines.push(`  ${protocolLabel(protocol)}: ${operations.length} evidence-backed operation(s)`, `    ${kinds.map(terminalSafe).join(" -> ")}`);
    }
    lines.push("", "SECURITY INVARIANTS", `  Proven secure: ${analysis.metrics.provenSecure}`, `  Proven insecure: ${analysis.metrics.provenInsecure}`, `  Applicable but unknown: ${analysis.metrics.unknown}`, `  Applicability unknown: ${applicabilityUnknown.length}`, `  Not applicable: ${analysis.metrics.notApplicable}`, `  Evidence steps: ${analysis.metrics.evidenceSteps}`);
    for (const result of analysis.results.filter((candidate) => candidate.applicability !== "NOT_APPLICABLE")) {
        lines.push(`  ${result.conclusion}  ${terminalSafe(result.invariantId)}`, `    Applicability: ${result.applicability}  Confidence: ${result.confidence}`, `    Evidence: ${result.evidencePath.length} step(s) across ${new Set(result.evidencePath.map((step) => step.location.path)).size} file(s)`, `    ${terminalSafe(result.explanation)}`);
    }
    if (applicableUnknown.length > 0 || applicabilityUnknown.length > 0) {
        ...
        for (const result of [...applicableUnknown, ...applicabilityUnknown]) {
            lines.push(`  ${terminalSafe(result.invariantId)}: ${terminalSafe(result.unresolvedConditions.join("; ") || result.explanation)}`);
        }
    }
    lines.push("", "FINDINGS");
    ...
    lines.push("COVERAGE", ...analysis.limitations.map((limitation) => `  - ${terminalSafe(limitation)}`));
    return `${lines.join("\n").trimEnd()}\n`;
}
export function renderAuthenticationGraphText(report) {
    const lines = [
    ...
    for (const node of report.authGraph.nodes) {
        ...
        lines.push(`  ${terminalSafe(node.id)}  ${terminalSafe(node.kind)}  ${terminalSafe(node.label)}`, source === undefined
            ? "    Source: unavailable"
    ...
    for (const edge of report.authGraph.edges) {
        lines.push(`  ${terminalSafe(edge.from)} --${terminalSafe(edge.relation)}--> ${terminalSafe(edge.to)}`, `    ${terminalSafe(edge.evidence)}`);
    }
