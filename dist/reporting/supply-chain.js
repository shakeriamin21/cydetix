...
export function renderSupplyChainText(analysis) {
    const fullPins = analysis.ci.actionReferences.filter((reference) => reference.pinning === "full-sha").length;
    const localPins = analysis.ci.actionReferences.filter((reference) => reference.pinning === "local").length;
    const historical = analysis.secrets.exposures.filter((exposure) => exposure.sourceCategory === "git-history").length;
    return `${[
        "SUPPLY CHAIN",
        "",
        "Dependencies",
