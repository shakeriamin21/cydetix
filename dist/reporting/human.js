...
export function decisionCategory(finding) {
    if (finding.proofState === "UNKNOWN")
        return "UNKNOWN";
    if (finding.confidence === "low" || finding.reachability === "unknown")
        return "REVIEW";
    if (SEVERITY_RANK[finding.severity] >= SEVERITY_RANK.high)
        return "FIX NOW";
    return "REVIEW";
}
function relevantAreas(report) {
    const areas = new Set();
    if (report.manifest.filesExamined > 0)
        areas.add("Application");
    if (report.manifest.authenticationLibraries.length > 0 ||
        report.manifest.sessionAndTokenTechnology.length > 0 ||
        report.manifest.oauthOidcProviders.length > 0)
        areas.add("Authentication");
    if (report.securityAnalysis.authorizationProofs.length > 0)
...
export function renderHuman(report) {
    ...
    for (const area of relevantAreas(report))
        lines.push(`  ${area}`);
    lines.push("", `Security status: ${status}`, "", `FIX NOW   ${counts["FIX NOW"]}`, `REVIEW    ${counts.REVIEW}`, `UNKNOWN   ${counts.UNKNOWN}`, "Urgency is separate from remediation authority. UNKNOWN counts proof instances, not confirmed vulnerabilities.", "", ...renderUncertainty(report, 3));
    if (shown.length > 0)
        lines.push("");
    ...
    if (other > 0) {
        lines.push(`${other} additional findings. Run cydetix --details or --json for complete evidence.`, "");
    }
    if (safe > 0)
...
export function renderRemediationHuman(report) {
    ...
    if (report.dryRun) {
        ...
        lines.push(`${report.summary.verified} SAFE ${report.summary.verified === 1 ? "fix was" : "fixes were"} applied and verified.`, "");
    }
    else {
    ...
    if (!report.dryRun && report.summary.verificationFailed > 0)
        lines.push("", "Verification failed. Inspect each transaction's final state and rollback evidence before retrying.");
