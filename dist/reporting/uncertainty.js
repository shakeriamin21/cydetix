...
export function unknownEvidence(report) {
    return [
        ...report.findings
            .filter((finding) => finding.proofState === "UNKNOWN")
            .map((finding) => `${finding.ruleId} ${finding.location.path}:${finding.location.start.line}: ${finding.evidence.map((item) => item.message).join(" ")}`),
        ...(report.securityAnalysis.applicationDataflow?.unknowns ?? []).map((item) => `${item.ruleId} ${item.path}:${item.line}: ${item.explanation}`),
        ...report.securityAnalysis.authorizationProofs
            .filter((item) => item.state === "UNKNOWN")
            .map((item) => `${item.invariant} ${item.evidencePath[0]?.location.path ?? "unresolved path"}: ${item.explanation}`),
        ...(report.securityAnalysis.authenticationAnalysis?.results ?? [])
            .filter((item) => item.applicability === "UNKNOWN" ||
            (item.applicability === "APPLICABLE" && item.conclusion === "UNKNOWN"))
            .map((item) => `${item.invariantId}: ${item.explanation} ${item.unresolvedConditions.join(" ")}`),
    ];
}
export function renderUncertainty(report, limit = Number.POSITIVE_INFINITY) {
