...
function nodeId(kind, label, sourcePath, stableKey) {
...
function fileEvidence(file, message) {
    return [
        {
            location: {
...
function operationNodeKind(operation) {
    const mapping = {
...
function operationRelation(operation) {
    if (operation.kind.includes("Revoke") || operation.kind === "PasswordResetConsume") {
        return "revokes";
    }
    if (operation.kind.includes("Rotate"))
        return "rotates";
    if (operation.kind.includes("Validate") || operation.kind === "JWTVerify")
        return "validates";
    if (operation.kind.includes("Create") || operation.kind.includes("Issue"))
        return "creates";
    if (operation.kind === "CredentialVerifier")
        return "verifies";
    if (operation.kind === "SessionLookup" || operation.kind === "TokenAccept")
        return "consumes";
    return "links";
}
export function buildAuthenticationGraph(manifest, files, securityIr, authorizationProofs = [], authenticationAnalysis) {
    const nodes = [];
    const edges = [];
    const seen = new Set();
    const addNode = (kind, label, confidence, sourcePath, sourceEvidence = [], stableKey) => {
    ...
    const identity = anchorFile === undefined
        ? undefined
        : addNode("identity", "application user", "medium", anchorFile.relativePath, fileEvidence(anchorFile, "Application identity graph anchor from repository evidence."));
    for (const technology of manifest.sessionAndTokenTechnology) {
        const lower = technology.toLowerCase();
        const kind = lower.includes("jwt") || lower.includes("jose") ? "jwt" : "session";
        const token = addNode(kind, technology, "high", packageFile?.relativePath, packageFile === undefined
            ? []
    ...
    for (const provider of manifest.oauthOidcProviders) {
        const oauth = addNode("oauth", provider, "medium", packageFile?.relativePath, packageFile === undefined
            ? []
    ...
    const addRoute = (route, sourcePath, label = route, sourceEvidence = [], stableKey) => {
        const lower = route.toLowerCase();
        let kind = "resource";
        if (/log-?out|sign-?out/.test(lower))
            kind = "logout-endpoint";
        else if (/reset|forgot/.test(lower))
            kind = "password-reset";
        else if (/register|sign-?up/.test(lower))
            kind = "registration-endpoint";
        else if (/log-?in|sign-?in|auth/.test(lower))
            kind = "login-endpoint";
    ...
    if (securityIr === undefined) {
        const routePattern = /(?:\b(?:app|router)\.(?:get|post|put|patch|delete)|@(?:app|router)\.(?:get|post|put|patch|delete))\s*\(\s*["']([^"']+)["']/g;
        for (const file of files) {
            if (!SUPPORTED_SOURCE_LANGUAGES.has(file.language))
                continue;
            for (const match of file.text.matchAll(routePattern)) {
                const route = match[1];
                if (route !== undefined) {
                    addRoute(route, file.relativePath, route, fileEvidence(file, "Literal route declaration."));
                }
            }
        }
    }
        ...
        for (const route of securityIr.routes) {
            routeNodes.set(route.id, addRoute(route.path, route.location.path, `${route.method} ${route.path}`, [
                {
        ...
        for (const fact of securityIr.identities) {
            subjectNodes.set(fact.id, addNode("subject", `${fact.name} [${fact.trust}]`, "high", fact.location.path, [{ location: fact.location, message: `Identity fact is classified as ${fact.trust}.` }], fact.id));
        }
        for (const operation of securityIr.resourceOperations) {
            resourceNodes.set(operation.id, addNode("resource", `${operation.technology} ${operation.resourceType}.${operation.operation}`, "high", operation.location.path, [
                {
        ...
        for (const proof of authorizationProofs) {
            ...
            if (routeNode === undefined || resourceNode === undefined)
                continue;
            const kind = proof.invariant === "tenant-isolation" ? "tenant-scope" : "ownership-check";
            const lastEvidence = proof.evidencePath.at(-1);
            const enforcement = addNode(kind, `${proof.invariant}: ${proof.state}`, proof.confidence, lastEvidence?.location.path, lastEvidence === undefined
                ? []
                : [
                    {
        ...
        if (authenticationAnalysis !== undefined) {
            ...
            for (const operation of authenticationAnalysis.operations) {
                const operationNode = addNode(operationNodeKind(operation), operation.kind, operation.confidence, operation.location.path, [{ location: operation.location, message: operation.guarantee }], operation.id);
            ...
            for (const result of authenticationAnalysis.results) {
                ...
                if (last === undefined || result.applicability === "NOT_APPLICABLE")
                    continue;
                const proofNode = addNode("enforcement-point", `${result.invariantId}: ${result.conclusion}`, result.confidence, last.location.path, [{ location: last.location, message: result.explanation }], result.id);
