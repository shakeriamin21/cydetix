...
`SOURCE_MODIFIED` and `VULNERABILITY_REMEDIATED` are different facts. VibeShield may say
"remediation applied and verified" only for an `APPLIED_VERIFIED` transaction whose structural
checks, authorized verification, deterministic rescan, and security-invariant postcondition all
passed. A written patch, absent fingerprint, or successful build alone is insufficient.
...
- `REVIEW_REQUIRED`: VibeShield produces a useful plan, but application/deployment semantics or an
  authoritative external resolution require a human decision. `--safe` never applies it.
- `ARCHITECTURAL`: remediation requires policy, migration, provider, data, key, trust-boundary, or
  incident-response decisions. VibeShield records a design and residual risk, not a source rewrite.
...
`vibeshield fix <root>` plans by default. It reports affected files, preconditions, transformations,
unified diff where deterministic, invariant, verification stages, classification, and residual risk
without writing. `--dry-run` is rigorously zero-write. Mutation requires `--safe`; noninteractive
mode applies only SAFE candidates and never prompts or promotes another class.

Before mutation, VibeShield records repository/Git identity and hashes for every affected file,
rejects an affected dirty file, validates canonical containment and file type, rejects symlinks and
path traversal, checks the exact vulnerable range hash, and prepares all non-overlapping
transformations in memory. A changed whole-file or range precondition returns `STALE_FINDING` and
requires a rescan. Unrelated dirty files are recorded and preserved.
...
bytes remain in transaction memory only; VibeShield does not create repository-wide backups.

## Verification and rollback

The current pipeline performs patch-structure validation, parser validation, optional explicitly
authorized commands, deterministic VibeShield rescan, and security-invariant proof. Rules declare a
...
If parser, command, rescan, or invariant verification fails, VibeShield rolls back only files whose
...
`ROLLBACK_FAILED` requires immediate review because VibeShield will not overwrite concurrent work.
...
- Mutable GitHub Action references remain REVIEW_REQUIRED unless an authoritative full SHA is
  supplied; VibeShield never invents or silently resolves one.
- Dependency vulnerabilities produce upgrade/lockfile-resolution plans. VibeShield does not edit a
