...
export const cycloneDx17Schema = z
  .object({
    ...
    dependencies: z.array(
      z.object({ ref: z.string().min(1), dependsOn: z.array(z.string().min(1)) }).strict(),
    ),
...
function decodePurlRoot(purl: string): { name: string; version: string } {
...
export function generateCycloneDxSbom(
  ...
  const components = [...byPurl.values()]
    ...
    .map((component) => ({
      ...
      purl: component.purl,
      scope: component.optional ? ("optional" as const) : ("required" as const),
      properties: [
  ...
  const dependenciesByRef = new Map<string, Set<string>>();
  ...
  for (const component of components) {
    if (!dependenciesByRef.has(component.purl)) dependenciesByRef.set(component.purl, new Set());
  }
  const assemblies = [
