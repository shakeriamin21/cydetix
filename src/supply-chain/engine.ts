...
import { analyzeAdvisories, type AdvisoryOptions } from "./advisories.js";
import { analyzeGithubActions, type WorkflowSignal } from "./github-actions.js";
import { scanGitHistory } from "./history.js";
import {
  supplyChainAnalysisSchema,
...
function secretEvidence(
  exposures: SupplyChainAnalysis["secrets"]["exposures"],
): SupplyChainEvidence[] {
  return exposures.map((exposure) =>
    supplyChainEvidenceSchema.parse({
      ...
      message:
        exposure.sourceCategory === "git-history"
...
export async function buildSupplyChainAnalysis(
  files: readonly SourceFile[],
  options: SupplyChainOptions,
): Promise<SupplyChainBuildResult> {
  ...
  const exposures = [
    ...new Map(
      [...workingTreeSecrets.exposures, ...historicalExposures].map((exposure) => [
  ...
  const secrets = {
    ...workingTreeSecrets,
    history: history?.state ?? ("NOT_CHECKED" as const),
    exposures,
    limitations: [
  ...
  const allActionPinsSecure =
    workflows.analysis.actionReferences.length > 0 &&
    workflows.analysis.actionReferences.every((reference) =>
      ["full-sha", "local", "digest"].includes(reference.pinning),
    );
  const hasWriteAll = workflows.analysis.permissions.some(
    (permission) => permission.access === "write-all",
  );
  ...
  const analysis = supplyChainAnalysisSchema.parse({
    ...
    controls: {
      sourceIntegrity:
        workflows.analysis.actionReferences.length === 0
          ...
          : allActionPinsSecure
            ...
            : "PARTIAL",
      buildProvenance:
        workflows.analysis.provenanceWorkflows.length > 0 ? "PARTIAL" : "NOT_PRESENT",
      ...
      dependencyInventory:
        inventoryResult.inventory.status === "COMPLETE"
          ? "PROVEN"
