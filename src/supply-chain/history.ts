...
function runGit(root: string, args: readonly string[], maxBuffer: number) {
  return spawnSync(
    "git",
    [
      "-C",
...
function safeGitPath(candidate: string): string | undefined {
...
export function scanGitHistory(root: string, maximumBytes = 32 * 1024 * 1024): HistoryScanResult {
  ...
  const truncated =
    history.error !== undefined && "code" in history.error && history.error.code === "ENOBUFS";
