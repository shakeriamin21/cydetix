...
import type { Finding, FixEdit, ScanReport } from "../core/schema.js";
import {
  createBoundary,
  isWithinRoot,
...
import type { SourceFile, SourceLanguage } from "../repository-discovery/traverse.js";
import { detectSecretsInText } from "../supply-chain/secrets.js";
import { createLocalExplicitRunner, type VerificationRunner } from "../verification/runner.js";
import type { VerificationExecutionResult } from "../verification/model.js";
import {
  remediationCandidateSchema,
  remediationReportSchema,
  remediationTransactionSchema,
  type ActualChange,
  type FindingStateTransition,
  type PlannedTransformation,
...
export function inspectGitState(root: string): GitState {
...
function languageFor(filePath: string): SourceLanguage {
...
function verificationScope(finding: Finding): VerificationScope {
  if (finding.category === "ci-cd") return "WORKFLOW";
  if (finding.category === "dependency-security") return "DEPENDENCY_GRAPH";
  if (["authentication", "password-reset", "oauth"].includes(finding.category)) return "AUTH_FLOW";
  if (finding.evidencePath !== undefined) return "MODULE";
  return "FILE";
}
...
function redactForDiff(filePath: string, text: string): string {
  let redacted = text;
  const unique = new Map<string, ReturnType<typeof detectSecretsInText>[number]>();
  for (const exposure of detectSecretsInText(text, {
    ...
    if (!unique.has(key)) unique.set(key, exposure);
  }
  const exposures = [...unique.values()].sort(
    (left, right) => right.location.start.offset - left.location.start.offset,
  );
  let nextStart = Number.POSITIVE_INFINITY;
  for (const exposure of exposures) {
    if (exposure.location.end.offset > nextStart) continue;
...
export function createUnifiedDiff(filePath: string, before: string, after: string): string {
  ...
  const body: string[] = [];
  for (let index = contextStart; index < prefix; index += 1) body.push(` ${oldLines[index] ?? ""}`);
...
function applyEdits(text: string, edits: readonly FixEdit[]): string {
  let updated = text;
  for (const edit of [...edits].sort((left, right) => right.startOffset - left.startOffset)) {
    if (edit.startOffset > edit.endOffset || edit.endOffset > updated.length)
      throw new InvariantSecError(
        `Invalid remediation offsets for ${edit.path}.`,
    ...
    if (sha256(current) !== edit.expectedTextSha256)
      throw new InvariantSecError(
        `Remediation precondition changed for ${edit.path}; rescan required.`,
        EXIT.verificationFailure,
      );
...
function planOnlyAdapter(finding: Finding): PlannedTransformation["adapter"] {
  if (finding.ruleId === "AS-CI-001") return "github-action-pin-plan-v1";
  if (finding.ruleId === "AS-SCA-001") return "dependency-upgrade-plan-v1";
  if (finding.ruleId === "AS-SECRET-001") return "secret-incident-plan-v1";
  return finding.autofix === "ARCHITECTURAL" ? "architectural-plan-v1" : "review-plan-v1";
}

function remediationSteps(finding: Finding): RemediationCandidate["remediationSteps"] {
  ...
  if (finding.ruleId === "AS-CI-001") return ["ACTION_SHA_RESOLUTION_REQUIRED"];
  if (finding.autofix === "ARCHITECTURAL") return ["ARCHITECTURE_CHANGE_REQUIRED"];
  if (finding.autofix === "REVIEW_REQUIRED") return ["BUSINESS_POLICY_REVIEW"];
  return [];
}

function residualRisk(finding: Finding): string[] {
  if (finding.ruleId === "AS-SECRET-001")
    return [
...
function expectedInvariant(finding: Finding): string {
...
async function buildCandidate(
  ...
  finding: Finding,
  gitState: GitState,
  authoritativeActionPins: Readonly<Record<string, string>>,
  externalCommandsAuthorized: boolean,
  ...
  const state =
    finding.autofix === "SAFE" && finding.fix !== undefined && !fileIsDirty
      ? "PLANNED"
  ...
  return remediationCandidateSchema.parse({
    ...
    rollbackStrategy:
      "Keep original bytes in transaction memory and restore only VibeShield-written files whose post-write hashes still match.",
    remediationSteps: remediationSteps(finding),
    residualRisk: residualRisk(finding),
  });
}

async function planContext(options: RemediationOptions): Promise<PlanningContext> {
  ...
  const selected = report.findings.filter(
    (finding) => options.finding === undefined || finding.fingerprint === options.finding,
  );
  ...
  const candidates = await Promise.all(
    selected.map((finding) =>
      buildCandidate(
        ...
        finding,
        gitState,
        options.authoritativeActionPins ?? {},
        (options.verificationCommands?.length ?? 0) > 0,
      ),
    ),
  );
  const plans = [
...
function exactTransformations(transformations: readonly PlannedTransformation[]) {
  return transformations.filter(
    (
      item,
    ): item is PlannedTransformation & {
      startOffset: number;
      endOffset: number;
      expectedTextSha256: string;
      replacement: string;
    } =>
      item.kind === "TEXT_REPLACEMENT" &&
...
function verifyNoOverlaps(transformations: readonly PlannedTransformation[]): void {
  const byPath = new Map<string, ReturnType<typeof exactTransformations>>();
  ...
  for (const [filePath, items] of byPath) {
    ...
    for (let index = 1; index < ascending.length; index += 1) {
      ...
      if (
        ...
        throw new InvariantSecError(
          `Refusing overlapping remediation edits in ${filePath}.`,
          EXIT.verificationFailure,
        );
    }
  }
...
async function prepareFiles(
  ...
  for (const [filePath, edits] of byPath) {
    ...
    if (baseline === undefined)
      throw new InvariantSecError(
        `Missing file baseline for ${filePath}.`,
        EXIT.verificationFailure,
      );
    ...
    if (!stat.isFile() || stat.isSymbolicLink())
      throw new InvariantSecError(
        `Remediation target is not a regular file: ${filePath}`,
        EXIT.verificationFailure,
      );
    const canonical = await realpath(target);
    if (!isWithinRoot(boundary.root, canonical))
      throw new InvariantSecError(
        `Remediation target escapes repository root: ${filePath}`,
        EXIT.verificationFailure,
      );
    const before = await readRegularFileInside(boundary, filePath, MAX_FIX_FILE_BYTES);
    if (sha256(before) !== baseline.sha256)
      throw new InvariantSecError(
        `STALE_FINDING: ${filePath} changed after planning; rescan required.`,
        EXIT.verificationFailure,
      );
...
async function atomicReplace(
  ...
  if (!isWithinRoot(boundary.root, temporary))
    throw new InvariantSecError(
      "Temporary remediation path escaped the repository.",
      EXIT.verificationFailure,
    );
  try {
    ...
    if (
      ...
      throw new InvariantSecError(
        `File identity changed during remediation: ${file.path}.`,
        EXIT.verificationFailure,
      );
  ...
  if (!written.equals(content))
    throw new InvariantSecError(
      `Atomic remediation write could not be verified: ${file.path}`,
      EXIT.verificationFailure,
    );
}
...
async function parserVerification(
  root: string,
  ...
  for (const file of files) {
    const text = (await readRegularFileInside(boundary, file.path, MAX_FIX_FILE_BYTES)).toString(
      "utf8",
    );
    ...
    if (parsed === undefined || isParseFailure(parsed))
      return result(
        "PARSER",
        "FAILED",
        parsed === undefined ? `No parser supports ${file.path}.` : parsed.message,
        started,
      );
  }
...
async function runTrustedCommand(
  ...
  if (command.executable.trim() === "" || command.executable.includes("\0"))
    return result(
      "TRUSTED_COMMAND",
      "FAILED",
      "Authorized command executable is invalid.",
      started,
    );
  const execution = await runner.run(root, {
    ...
    timeoutMilliseconds: Math.min(
      ...
      Math.max(1, command.timeoutMilliseconds ?? 30_000),
    ),
...
async function rollbackFiles(root: string, written: readonly PreparedFile[]) {
  const started = performance.now();
  try {
    ...
    return {
      ...
      verification: result(
        "ROLLBACK",
        "PASSED",
        "VibeShield restored only its own in-memory transaction bytes.",
        started,
      ),
    };
  } catch {
    return {
      ...
      verification: result(
        "ROLLBACK",
        "FAILED",
        "Rollback refused or failed because a target no longer matched VibeShield's written hash.",
        started,
      ),
    };
  }
...
function reportSummary(
  plans: readonly RemediationCandidate[],
  transactions: readonly RemediationTransaction[],
  residualFindings: number,
): RemediationReport["summary"] {
  return {
...
export async function executeSafeTransaction(
  root: string,
  plansInput: readonly RemediationCandidate[],
  context: Pick<PlanningContext, "gitState" | "repositoryIdentity" | "planningMilliseconds">,
  commands: readonly TrustedVerificationCommand[] = [],
  scanOptions: Pick<ScanOptions, "advisories" | "advisoryProvider"> = {},
  runtime: RemediationRuntime = {},
): Promise<RemediationTransaction> {
  ...
  if (plans.some((plan) => plan.classification !== "SAFE" || plan.state !== "PLANNED"))
    throw new InvariantSecError(
      "Unsafe or unapproved remediation cannot enter a SAFE transaction.",
      EXIT.unsafeRemediation,
    );
  ...
  try {
    ...
    verificationResults.push(
      result(
        "PRECONDITION",
        "PASSED",
        "Canonical paths, file types, whole-file hashes, and vulnerable ranges match the plan.",
        transformationStart,
      ),
    );
  } catch (error) {
    ...
    verificationResults.push(
      result(
        ...
        stale
          ...
          : "Remediation precondition failed before any write.",
        transformationStart,
      ),
    );
    return failedBeforeWrite(
      ...
      stale,
      startedAt,
      context.planningMilliseconds,
      performance.now() - transformationStart,
    );
  }
  const transformationMilliseconds = performance.now() - transformationStart;
  const actualChanges: ActualChange[] = prepared.map((file) => ({
    path: file.path,
  ...
  verificationResults.push(
    result(
      "PATCH_STRUCTURE",
      "PASSED",
      "All transformations are bounded, non-overlapping, and matched their source ranges.",
      transformationStart,
    ),
  );
  ...
  try {
    ...
    verificationResults.push(
      result(
        "PATCH_STRUCTURE",
        "FAILED",
        "A file transaction failed; verification did not begin.",
        fileTransactionStart,
      ),
      rollback.verification,
    );
    return remediationTransactionSchema.parse({
      ...base,
      actualChanges: actualChanges.filter((change) =>
        written.some((file) => file.path === change.path),
      ),
      verificationResults,
  ...
  if (parser.status === "PASSED" && commands.length === 0)
    verificationResults.push(
      result(
        "TRUSTED_COMMAND",
        "NOT_AUTHORIZED",
        "No external command was authorized; repository scripts were not executed.",
        performance.now(),
      ),
    );
  ...
  const preRescanFailed = verificationResults.some((item) =>
    ["FAILED", "UNAVAILABLE"].includes(item.status),
  );
  ...
  if (!preRescanFailed) {
    ...
    try {
      ...
      verificationResults.push(
        result(
          "TARGETED_RESCAN",
          remaining.length === 0 ? "PASSED" : "FAILED",
          remaining.length === 0
            ? "Correctness-first repository rescan no longer proves the targeted finding."
            : "The targeted finding remains provable after the patch.",
          rescanStart,
        ),
      );
      const boundary = await createBoundary(root);
      const hashesMatch = await Promise.all(
        prepared.map(
          async (file) =>
            sha256(await readRegularFileInside(boundary, file.path, MAX_FIX_FILE_BYTES)) ===
            sha256(file.after),
        ),
      );
      ...
      verificationResults.push(
        result(
          ...
          secure
            ...
            : "The intended invariant could not be proven after remediation.",
          performance.now(),
        ),
      );
    } catch {
      ...
      verificationResults.push(
        result(
          "TARGETED_RESCAN",
          "FAILED",
          "Security rescan failed safely; absence of a finding was not inferred.",
          rescanStart,
        ),
      );
    }
...
export async function planRemediations(options: RemediationOptions): Promise<RemediationReport> {
...
export async function runRemediation(
  ...
  const applicable = context.plans.filter(
    (plan) => plan.classification === "SAFE" && plan.state === "PLANNED",
  );
  const transactions =
    applicable.length === 0
      ? []
