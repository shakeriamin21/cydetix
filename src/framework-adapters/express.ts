...
function provesAuthentication(symbolSource: string, requestName: string): boolean {
  const request = escapeRegExp(requestName);
  const verifiesToken =
    /\b(?:jwt|jsonwebtoken)\.verify\s*\(/.test(symbolSource) ||
    /\b(?:jwtVerify|verifyToken)\s*\(/.test(symbolSource);
  const readsCredential = new RegExp(
    `\\b${request}(?:\\?\\.)?\\.(?:headers(?:\\?\\.)?\\.?authorization|get\\(\\s*["']authorization["']\\s*\\))`,
    "i",
  ).test(symbolSource);
  const establishesContext = new RegExp(`\\b${request}(?:\\?\\.)?\\.(?:auth|user)\\s*=`).test(
    symbolSource,
  );
  return verifiesToken && readsCredential && establishesContext;
}

export function analyzeExpressAuthentication(
  ir: SecurityIr,
  files: readonly SourceFile[],
): ExpressAuthenticationProof[] {
  ...
  const proofs: ExpressAuthenticationProof[] = [];
  ...
  for (const route of ir.routes) {
    ...
    for (const middlewareId of route.middlewareSymbolIds) {
      ...
      if (file === undefined || requestName === undefined) continue;
      const source = file.text.slice(
        middleware.location.start.offset,
        middleware.location.end.offset,
      );
      if (!provesAuthentication(source, requestName)) continue;
      proofs.push({
        routeId: route.id,
  ...
  return proofs.sort((left, right) =>
    `${left.routeId}:${left.middlewareSymbolId}`.localeCompare(
      `${right.routeId}:${right.middlewareSymbolId}`,
    ),
  );
}
