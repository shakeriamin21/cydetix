...
interface FlowStep {
  readonly kind: "SOURCE" | "PROPAGATION" | "TRANSFORMATION";
...
function createCandidate(
  ...
  if (typeof call.start !== "number" || typeof call.end !== "number") return undefined;
...
function analyzeJavaScriptFile(
  ...
  const markRouteArgument = (argument: Node | undefined): void => {
    ...
    if (argument.type === "FunctionExpression" || argument.type === "ArrowFunctionExpression") {
      ...
      if (info !== undefined) info.routeRoot = true;
      return;
    }
  ...
  traverse(parsed.ast, {
    CallExpression(path_) {
      ...
      if (
        ...
        for (const argument of path_.node.arguments.slice(1)) markRouteArgument(argument);
      }
...
interface PythonFact {
  ...
  readonly steps: readonly { label: string; offset: number }[];
  readonly unknownControl: boolean;
}
...
function analyzePythonFile(
  ...
  const routeFunctions: Array<{ from: number; to: number; parameters: string[] }> = [];
  ...
  for (const range of executableRanges.filter((item) => item.name === "CallExpression")) {
    ...
    const command =
      (imports.has("os") && /\bos\.system\s*\(/.test(callText)) ||
    ...
    if (filesystem) {
      ...
      if (!confined) {
        if (fact.unknownControl) unknown("AS-PATH-001", "Path confinement is UNKNOWN.");
        else
          addPythonCandidate(
            "PATH_TRAVERSAL",
            range,
    ...
    if (http) {
      ...
      if (!policy) {
        if (fact.unknownControl) unknown("AS-SSRF-001", "Network destination policy is UNKNOWN.");
        else
          addPythonCandidate(
            "SSRF",
            range,
