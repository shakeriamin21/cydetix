...
export function validateBetaReadiness(value: unknown): BetaReadiness {
  ...
  if (
    new Set(report.corpus.identities.map((item) => item.repository)).size !==
    report.corpus.identities.length
  )
    throw new Error("Duplicate corpus identity.");
  if (
    new Set(report.supplyChainGates.map((item) => item.id)).size !== report.supplyChainGates.length
  )
  ...
  if (
    report.tests.sandboxPassed > report.tests.passed ||
  ...
  if (report.verdict === "ALPHA12_BETA_READY_WITH_LIMITATIONS") {
    ...
    for (const size of ["small", "medium", "large"])
      if (
        !report.performance.measurements.some(
          (item) =>
            item.size === size &&
