...
import { INTEGRATION_ADAPTERS, integrationContext, type SetupOptions } from "./setup.js";
...
export async function createAgentCompatibilityReport(
  options: Pick<SetupOptions, "projectRoot" | "homeDirectory" | "platform" | "executablePath"> = {},
): Promise<AgentCompatibilityReport> {
  ...
  const agents = await Promise.all(
    INTEGRATION_ADAPTERS.map(async (adapter) => {
      ...
      if (detection === undefined) throw new Error(`Missing agent detection: ${adapter.id}`);
...
export function renderAgentCompatibilityReport(report: AgentCompatibilityReport): string {
