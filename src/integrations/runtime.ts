...
export const PERSISTENT_RUNTIME_REQUIRED =
  'AI integration requires a persistent local Cydetix installation. Run "npm install -g cydetix", then rerun "cydetix setup". The normal "npx cydetix" scanner remains available.';
...
export function isEphemeralNpxPath(candidate: string): boolean {
...
async function packageRootFromThisModule(): Promise<string> {
  let current = path.dirname(fileURLToPath(import.meta.url));
  for (;;) {
    const manifest = path.join(current, "package.json");
    const metadata = await lstat(manifest).catch(() => undefined);
    if (metadata?.isFile() === true && !metadata.isSymbolicLink()) {
      const parsed = JSON.parse(await readFile(manifest, "utf8")) as RuntimePackageJson;
      if (parsed.name === "cydetix") return current;
    }
...
async function assertSafePackagePath(packageRoot: string, candidate: string): Promise<string> {
...
export async function verifyPersistentRuntime(options: {
  ...
  if (isEphemeralNpxPath(requestedPackageRoot)) throw new Error(PERSISTENT_RUNTIME_REQUIRED);
  ...
  if (isEphemeralNpxPath(packageRoot)) throw new Error(PERSISTENT_RUNTIME_REQUIRED);
  ...
  const packageJson = JSON.parse(await readFile(packageJsonPath, "utf8")) as RuntimePackageJson;
  ...
  if (packageJson.version !== options.expectedVersion)
    throw new Error(
      `Persistent Cydetix runtime version mismatch: required ${options.expectedVersion}, found ${String(packageJson.version)}.`,
    );
  if (
  ...
  const entrypointRelative = (packageJson.bin as Record<string, unknown>).cydetix;
  if (
  ...
  const entrypoint = await assertSafePackagePath(
    packageRoot,
    path.resolve(packageRoot, entrypointRelative),
  );
  ...
  if (!nodeMetadata.isFile()) throw new Error("The configured Node executable is not a file.");
