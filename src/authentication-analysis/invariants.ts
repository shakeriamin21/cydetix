...
import type { EvidencePathStep } from "../authorization-analysis/model.js";
...
import type { IrCall, IrEvidence, IrRoute, SecurityIr } from "../security-ir/model.js";
import {
  authenticationAnalysisSchema,
  authenticationInvariantResultSchema,
  authenticationInvariantSchema,
  type AuthenticationAnalysis,
  type AuthenticationInvariant,
  type AuthenticationInvariantId,
  type AuthenticationInvariantResult,
...
const ASVS_TOKEN =
  "https://github.com/OWASP/ASVS/blob/v5.0.0/5.0/en/0x18-V9-Self-contained-Tokens.md";
...
] satisfies AuthenticationInvariant[];

export const AUTHENTICATION_INVARIANTS: readonly AuthenticationInvariant[] =
  invariantDefinitions.map((definition) => authenticationInvariantSchema.parse(definition));

const MAX_PATH_STATES = 10_000;

function packageRoots(files: readonly SourceFile[]): string[] {
  return files
...
function packageScope(relativePath: string, roots: readonly string[]): string {
  return (
    roots.find(
...
function operationsByPackage(
  operations: readonly AuthenticationOperation[],
  roots: readonly string[],
  include: (operation: AuthenticationOperation) => boolean,
): Array<{ scope: string; operations: AuthenticationOperation[] }> {
  const groups = new Map<string, AuthenticationOperation[]>();
...
function callAdjacency(ir: SecurityIr): Map<string, IrCall[]> {
  const result = new Map<string, IrCall[]>();
...
function findCallPath(
  adjacency: ReadonlyMap<string, readonly IrCall[]>,
  start: string,
  target: string,
  maxDepth = 12,
): IrCall[] | undefined {
  if (start === target) return [];
  const pending: Array<{ symbolId: string; calls: IrCall[]; visited: Set<string> }> = [
  ...
  while (pending.length > 0 && explored < MAX_PATH_STATES) {
    ...
    explored += 1;
    if (current === undefined || current.calls.length >= maxDepth) continue;
    for (const call of adjacency.get(current.symbolId) ?? []) {
      const callee = call.calleeSymbolId;
      if (callee === undefined || current.visited.has(callee)) continue;
      const calls = [...current.calls, call];
      if (callee === target) return calls;
...
function resultId(invariantId: AuthenticationInvariantId, correlationKey: string): string {
  return `authproof:${stableFingerprint(["1.0.0", invariantId, correlationKey]).slice(0, 16)}`;
}

function evidenceFor(ir: SecurityIr, id: string): IrEvidence | undefined {
  return ir.evidence.find((item) => item.id === id);
}

function operationEvidenceKind(operation: AuthenticationOperation): EvidencePathStep["kind"] {
  if (operation.protocol === "session")
    return operation.kind.includes("Revoke") ? "revocation" : "session";
  if (operation.protocol === "password-reset") return "password-reset";
  if (operation.protocol === "jwt" || operation.protocol === "refresh-token") return "token";
  if (operation.protocol === "oauth" || operation.protocol === "oidc") return "oauth";
...
function evidencePath(
  ...
  operations: readonly AuthenticationOperation[],
  conclusion: AuthenticationInvariantResult["conclusion"],
  invariantId: AuthenticationInvariantId,
): EvidencePathStep[] {
  const adjacency = callAdjacency(ir);
  const raw: Array<Omit<EvidencePathStep, "order">> = [];
  const seen = new Set<string>();
  const push = (step: Omit<EvidencePathStep, "order">): void => {
  ...
  for (const route of routes) {
    ...
    for (const operation of operations.filter((item) => item.routeIds.includes(route.id))) {
      if (route.handlerSymbolId !== undefined && operation.functionSymbolId !== undefined) {
        for (const call of findCallPath(
          adjacency,
          route.handlerSymbolId,
          operation.functionSymbolId,
        ) ?? []) {
  ...
  if (anchor !== undefined && conclusion !== "UNKNOWN") {
    push({
      ...
      irId: resultId(
        invariantId,
        `${routes.map((route) => route.id).join(":")}:${operations.map((operation) => operation.id).join(":")}`,
      ),
      location: anchor.location,
      message:
        conclusion === "PROVEN_SECURE"
...
function makeResult(input: {
  ...
  readonly applicability: AuthenticationInvariantResult["applicability"];
  readonly conclusion: AuthenticationInvariantResult["conclusion"];
  readonly operations: readonly AuthenticationOperation[];
  readonly routes: readonly IrRoute[];
  readonly explanation: string;
  readonly unresolvedConditions?: readonly string[];
  readonly correlationKey: string;
  readonly ir: SecurityIr;
  readonly confidence?: AuthenticationInvariantResult["confidence"];
}): AuthenticationInvariantResult {
...
function attachRoutes(
  ...
  return operations.map((operation) => {
    if (operation.functionSymbolId === undefined) return operation;
    const routeIds = ir.routes
      .filter((route) => {
        const roots = [route.handlerSymbolId, ...route.middlewareSymbolIds].filter(
          (symbolId): symbolId is string => symbolId !== undefined,
        );
        return roots.some(
          (root) => findCallPath(adjacency, root, operation.functionSymbolId ?? "") !== undefined,
        );
      })
      .map((route) => route.id)
      .sort();
...
function derivedRouteOperations(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
): AuthenticationOperation[] {
  ...
  for (const route of ir.routes) {
    ...
    const add = (
      kind: AuthenticationOperation["kind"],
      protocol: AuthenticationOperation["protocol"],
      guarantee: string,
    ): void => {
    ...
    if (routeOperations.some((operation) => operation.kind === "OAuthStateValidation")) {
      add(
        "OAuthCallback",
        "oauth",
        "The resolved Express route contains OAuth authorization-response validation.",
      );
    }
    if (
      routeOperations.some((operation) => operation.kind === "PasswordResetCredentialIssue") &&
      !routeOperations.some((operation) => operation.kind === "PasswordChange")
    ) {
      add(
        "PasswordResetRequest",
        "password-reset",
        "The resolved Express route issues a persisted password-reset credential.",
      );
    }
    if (routeOperations.some((operation) => operation.kind === "CredentialVerifier")) {
      add(
        "CredentialInput",
        "session",
        "The resolved Express authentication route receives credentials for verification.",
      );
    }
  }
...
function evaluateSessionRotation(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
): AuthenticationInvariantResult[] {
  const results: AuthenticationInvariantResult[] = [];
  for (const route of ir.routes) {
    ...
    if (verifier === undefined || success === undefined) continue;
  ...
  return results.length === 0
    ? [
        notApplicable(
          ...
          "No supported stateful login upgrade was detected.",
          ir,
        ),
      ]
    : results;
...
function evaluateLogout(
  ...
  const results: AuthenticationInvariantResult[] = [];
  for (const route of ir.routes) {
    const revocations = operations.filter(
      (operation) => operation.routeIds.includes(route.id) && operation.kind === "SessionRevoke",
    );
    if (revocations.length === 0) continue;
    const authoritative = revocations.find(
      (operation) => operation.attributes.authoritative === "true",
    );
    const transportOnly = revocations.find(
      (operation) => operation.attributes.transportOnly === "true",
    );
    const conclusion =
  ...
  return results.length === 0
    ? [
        notApplicable(
          ...
          "No supported logout revocation boundary was detected.",
          ir,
        ),
      ]
    : results;
...
function passwordResetRoutes(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
): Array<{ route: IrRoute; operations: AuthenticationOperation[] }> {
  return ir.routes
    .map((route) => ({
      route,
    ...
    .filter(
      ({ operations: onRoute }) =>
        onRoute.some((operation) => operation.kind === "PasswordResetValidate") &&
        onRoute.some((operation) => operation.kind === "PasswordChange"),
    );
}

function evaluateResetCredentialProtection(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
  roots: readonly string[],
): AuthenticationInvariantResult[] {
  const requestFlows = ir.routes
    ...
    .filter(({ operations: onRoute }) =>
      onRoute.some((operation) => operation.kind === "PasswordResetRequest"),
    );
  if (requestFlows.length === 0) {
    return [
      notApplicable(
        "PASSWORD_RESET_CREDENTIAL_PROTECTED",
        "No supported application-owned password-reset credential issuance was detected.",
        ir,
      ),
    ];
  }
  return requestFlows.map(({ route, operations: onRoute }) => {
    ...
    const validations = operations.filter(
      (operation) =>
        operation.kind === "PasswordResetValidate" &&
        packageScope(operation.location.path, roots) === scope,
    );
    const persistence = onRoute.find(
      (operation) =>
        operation.kind === "PasswordResetCredentialIssue" &&
        operation.attributes.expiryStored !== undefined,
    );
    const generator = onRoute.find(
      (operation) => operation.attributes.credentialGeneration !== undefined,
    );
    const validation = validations[0];
    const secure =
      generator?.attributes.credentialGeneration === "cryptographic" &&
    ...
    const explicitlyWeak =
      generator?.attributes.credentialGeneration === "weak" ||
    ...
    const relatedRoutes = ir.routes.filter(
      (candidate) =>
...
function evaluateResetSingleUse(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
): AuthenticationInvariantResult[] {
  ...
  if (flows.length === 0) {
    return [
      notApplicable(
        ...
        "No supported password-reset consumption flow was detected.",
        ir,
      ),
    ];
  }
  return flows.map(({ route, operations: onRoute }) => {
    ...
    const selected = [validation, passwordChange, consume].filter(
      (operation): operation is AuthenticationOperation => operation !== undefined,
    );
    return makeResult({
...
function evaluateResetSessions(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
  roots: readonly string[],
): AuthenticationInvariantResult[] {
  ...
  if (flows.length === 0) {
    return [
      notApplicable(
        ...
        "No supported password-reset password mutation was detected.",
        ir,
      ),
    ];
  }
  return flows.map(({ route, operations: onRoute }) => {
    const scope = packageScope(route.location.path, roots);
    const sessionLookups = operations.filter(
      (operation) =>
        operation.kind === "SessionLookup" &&
        packageScope(operation.location.path, roots) === scope,
    );
    const sessionCreates = operations.filter(
      (operation) =>
        operation.kind === "SessionCreate" &&
        packageScope(operation.location.path, roots) === scope,
    );
    const passwordChange = onRoute.find((operation) => operation.kind === "PasswordChange");
    ...
    const conclusion =
      revokeAll !== undefined || (versionChange && versionChecked)
        ? "PROVEN_SECURE"
        : independentSessions && !versionChange && !versionChecked
          ? "PROVEN_INSECURE"
          : "UNKNOWN";
    const selected = [
      ...onRoute.filter((operation) =>
        ["PasswordResetValidate", "PasswordChange", "SessionRevokeAll"].includes(operation.kind),
      ),
      ...sessionCreates,
      ...sessionLookups,
    ];
...
function jwtAcceptanceRoutes(
  ...
  return ir.routes
    ...
    .filter(({ operations: onRoute }) =>
      onRoute.some((operation) => operation.kind === "TokenAccept" && operation.protocol === "jwt"),
    );
}

function evaluateJwt(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
): AuthenticationInvariantResult[] {
  ...
  if (flows.length === 0) {
    return [
      notApplicable(
        ...
        "No supported JWT authenticated-context acceptance boundary was detected.",
        ir,
      ),
    ];
  }
  return flows.map(({ route, operations: onRoute }) => {
    const accept = onRoute.find(
      (operation) => operation.kind === "TokenAccept" && operation.protocol === "jwt",
    );
    ...
    const conclusion =
      verify !== undefined ? "PROVEN_SECURE" : decode !== undefined ? "PROVEN_INSECURE" : "UNKNOWN";
    return makeResult({
      invariantId: "JWT_SIGNATURE_VERIFIED",
      applicability: "APPLICABLE",
      conclusion,
      operations: [verify, decode, accept].filter(
        (operation): operation is AuthenticationOperation => operation !== undefined,
      ),
      routes: [route],
      explanation:
...
function evaluateJwtClaim(
  ir: SecurityIr,
  operations: readonly AuthenticationOperation[],
  invariantId: "JWT_EXPECTED_ISSUER_VALIDATED" | "JWT_EXPECTED_AUDIENCE_VALIDATED",
  attribute: "issuer" | "audience",
): AuthenticationInvariantResult[] {
  const flows = jwtAcceptanceRoutes(ir, operations);
  if (flows.length === 0) {
    return [
  ...
  return flows.map(({ route, operations: onRoute }) => {
    ...
    return makeResult({
      ...
      operations: [verify, accepted].filter(
        (operation): operation is AuthenticationOperation => operation !== undefined,
      ),
      routes: [route],
      explanation: proven
...
function evaluateRefresh(
  ...
  const groups = operationsByPackage(
    operations,
    roots,
    (operation) => operation.protocol === "refresh-token",
  );
  if (groups.length === 0) {
    return [
      notApplicable(
        "REFRESH_TOKEN_REPLAY_MITIGATED",
        "No supported refresh-token lifecycle was detected.",
        ir,
      ),
    ];
  }
  return groups.map(({ scope, operations: refresh }) => {
    const routes = ir.routes.filter((route) =>
      refresh.some((operation) => operation.routeIds.includes(route.id)),
    );
...
function evaluateOAuthState(
  ...
  const groups = operationsByPackage(
    operations,
    roots,
    (operation) => operation.protocol === "oauth" || operation.protocol === "oidc",
  );
  if (groups.length === 0) {
    return [
      notApplicable(
        "OAUTH_STATE_VALIDATED",
        "No supported OAuth authorization flow was detected.",
        ir,
      ),
    ];
  }
  return groups.map(({ scope, operations: oauth }) => {
    ...
    const stateGenerated = oauth.find(
      (operation) => operation.attributes.stateGenerated === "true",
    );
    ...
    const pkceComplete =
      oauth.some(
        (operation) =>
          operation.kind === "PKCEVerifier" && operation.attributes.stage === "challenge",
      ) &&
      oauth.some(
        (operation) =>
          operation.kind === "AuthorizationCodeExchange" &&
          operation.attributes.pkceBound === "true",
      );
    if (stateSent === undefined && validation === undefined && pkceComplete) {
      return notApplicable(
        "OAUTH_STATE_VALIDATED",
        `The supported ${scope || "root"} flow uses complete PKCE transaction binding instead of state.`,
        ir,
      );
    }
    const secure =
    ...
    const routes = ir.routes.filter((route) =>
      oauth.some((operation) => operation.routeIds.includes(route.id)),
    );
    return makeResult({
      ...
      operations: [stateGenerated, stateStored, stateSent, validation].filter(
        (operation): operation is AuthenticationOperation => operation !== undefined,
      ),
      routes,
      explanation: secure
...
function evaluatePkce(
  ...
  if (
    ...
    return [
      notApplicable(
        ...
        "No supported OAuth authorization-code exchange was detected.",
        ir,
      ),
    ];
  }
  return groups.flatMap(({ scope, operations: oauth }) => {
    ...
    const generated = oauth.find(
      (operation) =>
        operation.kind === "PKCEVerifier" && operation.attributes.stage === "generated",
    );
    const stored = oauth.find(
      (operation) => operation.kind === "PKCEVerifier" && operation.attributes.stage === "stored",
    );
    const challenge = oauth.find(
      (operation) =>
        operation.kind === "PKCEVerifier" && operation.attributes.stage === "challenge",
    );
    const challengeSent = oauth.find((operation) => operation.attributes.challengeSent === "true");
    const method = oauth.find(
      (operation) => operation.attributes.parameter === "code_challenge_method",
    );
    const secure =
      publicClient &&
      generated !== undefined &&
    ...
    return [
      makeResult({
        ...
        operations: [generated, stored, challenge, challengeSent, method, exchange].filter(
          (operation): operation is AuthenticationOperation => operation !== undefined,
        ),
        routes,
        explanation: secure
...
function evaluateOidcNonce(
  ...
  if (oidc.length === 0) {
    return [
      notApplicable(
        ...
        "No supported OIDC authentication flow was detected.",
        ir,
      ),
    ];
  }
  ...
  const routes = ir.routes.filter((route) =>
    oidc.some((operation) => operation.routeIds.includes(route.id)),
  );
  return [
    makeResult({
      ...
      operations: [sent, validated, accepted].filter(
        (operation): operation is AuthenticationOperation => operation !== undefined,
      ),
      routes,
      explanation:
...
export function buildAuthenticationAnalysis(
  ir: SecurityIr,
  ...
  const resetPersistenceRoutes = new Set(
    operations
      .filter(
        (operation) =>
          operation.kind === "PasswordResetCredentialIssue" &&
  ...
  operations = operations.filter(
    (operation) =>
      operation.attributes.credentialGeneration === undefined ||
