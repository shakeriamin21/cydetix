...
function securitySeverity(severity: Finding["severity"]): string {
  const values: Record<Finding["severity"], string> = {
    critical: "9.5",
    high: "8.0",
    medium: "5.5",
    low: "3.0",
...
function sarifRule(rule: RuleDefinition): unknown {
  return {
    id: rule.id,
...
function sarifResult(finding: Finding): unknown {
  const codeFlows =
        ...
        invocations: [
          {
            ...
            properties: {
              ...
              authenticationInvariantResults:
                report.securityAnalysis.authenticationAnalysis?.results.length ?? 0,
              authenticationProvenInsecure:
                report.securityAnalysis.authenticationAnalysis?.metrics.provenInsecure ?? 0,
              advisoryProviderState:
                report.securityAnalysis.supplyChainAnalysis?.advisories.state ?? "UNKNOWN",
              dependencyInventoryStatus:
              ...
              applicationDataflowUnknowns:
                report.securityAnalysis.applicationDataflow?.unknowns ?? [],
              authorizationUnknowns: report.securityAnalysis.authorizationProofs.filter(
                (item) => item.state === "UNKNOWN",
              ),
              authenticationUnknowns: (
