...
import type { IdentityFact, IrCall, IrEvidence, SecurityIr } from "../security-ir/model.js";
import {
  authorizationProofSchema,
  type AuthorizationProof,
  type EvidencePathStep,
} from "./model.js";

const OWNERSHIP_FIELD =
  /^(?:owner|ownerId|user|userId|account|accountId|member|memberId|subject|subjectId)$/i;
...
function proofId(routeId: string, resourceId: string, invariant: string): string {
...
function callAdjacency(ir: SecurityIr): Map<string, IrCall[]> {
  ...
  for (const calls of result.values()) calls.sort((left, right) => left.id.localeCompare(right.id));
  return result;
}
...
function identityLineage(ir: SecurityIr, fact: IdentityFact): IdentityFact[] {
  const byId = new Map(ir.identities.map((item) => [item.id, item]));
  const result: IdentityFact[] = [];
  const visiting = new Set<string>();
  const visit = (current: IdentityFact): void => {
...
function evidencePath(
  ...
  calls: readonly IrCall[],
  subject: IdentityFact | undefined,
  resource: SecurityIr["resourceOperations"][number],
  invariant: AuthorizationProof["invariant"],
...
export function buildAuthorizationProofs(ir: SecurityIr): AuthorizationProof[] {
  const proofs: AuthorizationProof[] = [];
  ...
  for (const route of ir.routes) {
    ...
    for (const resource of ir.resourceOperations) {
      ...
      const resourceFacts = ir.identities.filter(
        (identity) => identity.symbolId === resource.functionSymbolId,
      );
      const trustedSubjects = resourceFacts.filter(
        (identity) =>
          identity.trust === "trusted-authenticated" &&
          SUBJECT_IDENTITY.test(identity.name) &&
          !/tenant/i.test(identity.name),
      );
      const ownershipSelectors = resource.selectors.filter((selector) =>
        OWNERSHIP_FIELD.test(selector.field),
      );
      const provenSelector = ownershipSelectors.find(
        (selector) => selector.trust === "trusted-authenticated",
      );
      const attackerObjectSelector = resource.selectors.find(
        (selector) =>
          OBJECT_FIELD.test(selector.field) &&
          !OWNERSHIP_FIELD.test(selector.field) &&
      ...
      proofs.push(
        authorizationProofSchema.parse({
          schemaVersion: "1.0.0",
      ...
      const trustedTenants = resourceFacts.filter(
        (identity) => identity.trust === "trusted-authenticated" && /tenant/i.test(identity.name),
      );
      const tenantSelectors = resource.selectors.filter((selector) =>
        /^tenant(?:Id)?$/i.test(selector.field),
      );
      const trustedTenantSelector = tenantSelectors.find(
        (selector) => selector.trust === "trusted-authenticated",
      );
      const attackerTenantSelector = tenantSelectors.find(
      ...
      const tenantEnforcementMessage =
        tenantState === "PROVEN"
