...
import { runRemediation, type TrustedVerificationCommand } from "../remediation/fix.js";
import { remediationReportSchema } from "../remediation/model.js";
import { renderJson } from "../reporting/json.js";
import {
  renderAuthenticationGraphText,
  renderAuthenticationText,
} from "../reporting/authentication.js";
...
function confidence(value: string): Confidence {
  if (value in CONFIDENCE_RANK) return value as Confidence;
  throw new InvalidArgumentError("Expected one of: low, medium, high.");
}

function format(value: string): OutputFormat {
  if (["text", "json", "sarif"].includes(value)) return value as OutputFormat;
  throw new InvalidArgumentError("Expected one of: text, json, sarif.");
}

function graphFormat(value: string): GraphOutputFormat {
  if (["text", "json"].includes(value)) return value as GraphOutputFormat;
  throw new InvalidArgumentError("Expected one of: text, json.");
}

function advisoryMode(value: string): "offline" | "online" {
...
function trustedCommands(values: readonly string[]): TrustedVerificationCommand[] {
  return values.map((value) => {
    ...
    try {
      ...
      throw new InvalidArgumentError(
        '--verify-command must be a JSON string array such as ["npm","test"].',
      );
    }
    if (
...
function verificationRunner(mode: string, image: string | undefined): VerificationRunner {
  if (mode === "none") return createNoExecutionRunner();
  if (mode === "local") return createLocalExplicitRunner();
  if (mode === "container") {
    if (image === undefined || image.trim() === "")
      throw new InvalidArgumentError(
        "--sandbox-image with an immutable sha256 digest is required for container verification.",
      );
...
function recount(report: ScanReport): void {
  report.summary.critical = 0;
  report.summary.high = 0;
  report.summary.medium = 0;
  report.summary.low = 0;
  report.summary.info = 0;
  for (const finding of report.findings) report.summary[finding.severity] += 1;
}
...
function filterReport(
  ...
  report.findings = report.findings.filter(
    (finding) =>
      SEVERITY_RANK[finding.severity] >= SEVERITY_RANK[minimumSeverity] &&
...
function output(report: ScanReport, selectedFormat: OutputFormat): void {
  if (selectedFormat === "json") process.stdout.write(renderJson(report));
  else if (selectedFormat === "sarif") process.stdout.write(renderSarif(report));
...
async function executeScan(target: string, options: ScanCommandOptions): Promise<ScanReport> {
  if (!options.offline) {
    throw new CydetixError(
      "Cydetix is deliberately offline-only; network-backed adapters are not implemented.",
      EXIT.usage,
    );
  }
  try {
...
async function initRepository(target: string): Promise<void> {
  ...
  for (const name of [CONFIG_NAME]) {
    const existing = await lstat(path.join(boundary.root, name)).catch((error: unknown) => {
      if (errorCode(error) === "ENOENT") return undefined;
      throw error;
    });
    if (existing !== undefined) existingNames.push(name);
  }
  if (existingNames.length > 0) {
    throw new CydetixError(
      `${existingNames.join(" and ")} already exists; refusing to overwrite configuration.`,
      EXIT.usage,
    );
  }
  const config = {
...
export function buildProgram(): Command {
  ...
  program
    ...
    .exitOverride()
    .action(async () => {
      const options = program.opts<{ details: boolean; json: boolean; sarif: boolean }>();
      if (options.json && options.sarif)
        throw new CydetixError("Choose only one of --json or --sarif.", EXIT.usage);
      const report = await scanRepository({ path: "." });
      if (options.json) process.stdout.write(renderJson(report));
      else if (options.sarif) process.stdout.write(renderSarif(report));
      else if (options.details) process.stdout.write(renderText(report));
      else {
  ...
  program
    ...
    .action(async (target: string, options: ScanCommandOptions) => {
      if (options.nonInteractive) process.env.CYDETIX_AGENT_SUBPROCESS = "1";
  ...
  program
    ...
    .action(async (target: string, options: ScanCommandOptions) => {
      ...
      recount(report);
      if (options.format === "text") process.stdout.write(renderAuthenticationText(report));
      else output(report, options.format);
    });
  ...
  program
    ...
    .action(async (target: string, options: { auth: boolean; format: GraphOutputFormat }) => {
      if (!options.auth) {
      ...
      if (options.format === "json") {
        process.stdout.write(
          `${JSON.stringify(
            {
              schemaVersion:
                report.securityAnalysis.authenticationAnalysis?.graphVersion ?? "unavailable",
  ...
  program
    ...
    .action(
      async (
        target: string,
        options: { advisories: "offline" | "online"; format: GraphOutputFormat },
      ) => {
        ...
        if (options.format === "json") {
          process.stdout.write(
            `${JSON.stringify({ inventory: analysis.inventory, advisories: analysis.advisories }, null, 2)}\n`,
          );
        } else {
  ...
  program
    ...
    .option("--format <format>", "output format", graphFormat, "text")
    .action(async (target: string, options: { history: boolean; format: GraphOutputFormat }) => {
  ...
  program
    ...
    .action(
      async (
        ...
        if (analysis === undefined)
          throw new CydetixError("Supply-chain analysis unavailable.", EXIT.scanFailure);
        if (options.format === "text") process.stdout.write(renderSupplyChainText(analysis));
        else if (options.format === "json")
          process.stdout.write(`${JSON.stringify(analysis, null, 2)}\n`);
        else process.stdout.write(renderSarif(report));
      },
    );
  ...
  program
    ...
    .action(async (target: string, options: { format: GraphOutputFormat }) => {
      if (options.format !== "json") {
        throw new CydetixError(
          "CycloneDX SBOM output currently supports --format json only.",
          EXIT.usage,
        );
      }
      const report = await scanRepository({ path: target });
  ...
  program
    ...
    .option(
      "--verify-command <json>",
      "explicitly authorize one bounded non-shell verification command encoded as a JSON array",
      collectString,
      [],
    )
    .option(
      "--verification-runner <mode>",
      "explicit execution provider: none, local, or container",
      "local",
    )
    .option(
      "--sandbox-image <digest>",
      "locally present immutable image identity for container verification",
    )
    ...
    .action(
      async (
        ...
        if (
          ...
          result.transactions.some((transaction) => transaction.finalState === "APPLIED_VERIFIED")
        )
        ...
        else if (result.summary.residualFindings > 0) process.exitCode = EXIT.policyFindings;
      },
    );
  ...
  program
    ...
    .action(async (options: { projectRoot: string; requireVersion?: string }) => {
      await runMcpServer({
  ...
  program
    ...
    .option("--format <format>", "output format", format, "text")
    .action(async (target: string, options: { format: OutputFormat }) => {
      const report = await scanRepository({ path: target });
      output(report, options.format);
      if (
        report.findings.some((finding) => SEVERITY_RANK[finding.severity] >= SEVERITY_RANK.high)
      ) {
  ...
  program
    ...
    .option("--format <format>", "output format", graphFormat, "text")
    .action(async (reportPath: string, options: { format: GraphOutputFormat }) => {
      const absolute = path.resolve(reportPath);
      const stat = await lstat(absolute);
      if (!stat.isFile() || stat.isSymbolicLink() || stat.size > 10_485_760)
        throw new CydetixError(
          "Remediation report must be a regular file no larger than 10 MiB.",
          EXIT.usage,
        );
  ...
  program
    ...
    .action(async (target: string, options: { format: OutputFormat; failOn: Severity }) => {
      const report = await scanRepository({ path: target });
      output(report, options.format);
      if (
...
async function main(): Promise<void> {
  try {
    ...
    process.stderr.write(
      `Scan failed safely: ${terminalSafe(error instanceof Error ? error.message : String(error))}\n`,
    );
