...
import type { SourceFile, TraversalResult } from "./traverse.js";
...
const ORM_PACKAGES: Readonly<Record<string, string>> = {
  prisma: "Prisma",
...
function safePackageJson(file: SourceFile): PackageJson | undefined {
  if (path.basename(file.relativePath).toLowerCase() !== "package.json") return undefined;
  try {
...
function addDependencies(target: Set<string>, packageJson: PackageJson): void {
  for (const collection of [packageJson.dependencies, packageJson.devDependencies]) {
    if (collection === undefined) continue;
    for (const dependency of Object.keys(collection)) target.add(dependency.toLowerCase());
  }
}

function detectPythonPackages(files: readonly SourceFile[]): Set<string> {
  const packages = new Set<string>();
  for (const file of files) {
    const name = path.basename(file.relativePath).toLowerCase();
    if (!["requirements.txt", "pyproject.toml", "pipfile", "poetry.lock"].includes(name)) continue;
    ...
    for (const dependency of [
      ...
      if (
        new RegExp(`(^|[^a-z0-9_-])${dependency.replace("-", "[-_]?")}([^a-z0-9_-]|$)`, "m").test(
          lower,
        )
      ) {
...
function addLiteralRoutes(target: Set<string>, file: SourceFile): void {
  if (!["javascript", "typescript", "python"].includes(file.language)) return;
  const routePattern =
    /(?:\b(?:app|router)\.(get|post|put|patch|delete)|@(?:app|router)\.(get|post|put|patch|delete))\s*\(\s*["']([^"']+)["']/gi;
...
export function buildRepositoryManifest(traversal: TraversalResult): RepositoryManifest {
  const { files } = traversal;
  const dependencyNames = new Set<string>();
  const packageFiles: Array<{ file: SourceFile; parsed: PackageJson }> = [];
  for (const file of files) {
  ...
  for (const dependency of detectPythonPackages(files)) dependencyNames.add(dependency);
  ...
  const monorepoBoundaries = new Set<string>();
  ...
  for (const file of files) {
    ...
    if (
      ["dockerfile", "containerfile", "docker-compose.yml", "docker-compose.yaml"].includes(name)
    ) {
      infrastructure.add("Docker");
    }
    if (lower.includes("kubernetes/") || lower.includes("k8s/")) infrastructure.add("Kubernetes");
    if (lower.endsWith(".tf")) infrastructure.add("Terraform");
    ...
    if (/routes?|controllers?|pages\/api|app\/api/.test(lower)) apiRoutes.add(file.relativePath);
  ...
  for (const boundaryName of ["pnpm-workspace.yaml", "turbo.json", "nx.json"] as const) {
    const found = files.find((file) => file.relativePath.toLowerCase() === boundaryName);
    if (found !== undefined) monorepoBoundaries.add(".");
  }
  if (dependencyNames.has("vitest")) testFrameworks.add("Vitest");
  if (dependencyNames.has("jest")) testFrameworks.add("Jest");
  if (dependencyNames.has("pytest")) testFrameworks.add("pytest");

  const authenticationLibraries = sorted(
    [...dependencyNames].filter((name) => AUTH_PACKAGES.has(name)),
  );
  const ormAndDatabases = sorted(
    [...dependencyNames]
