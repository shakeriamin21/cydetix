...
function commandFingerprint(command: VerificationCommand): string {
...
function baseControls(kind: VerificationRunner["kind"]): SandboxCapability["controls"] {
  return {
...
function noExecutionCapability(): SandboxCapability {
...
export function createNoExecutionRunner(): VerificationRunner {
  return {
    kind: "NO_EXECUTION",
    capability() {
    ...
    run(_repositoryRoot, commandInput) {
      ...
      return Promise.resolve(
        verificationExecutionResultSchema.parse({
          schemaVersion: "1.0.0",
          runner: "NO_EXECUTION",
          state: "NOT_RUN",
...
export function createLocalExplicitRunner(): VerificationRunner {
  ...
  return {
    ...
    async run(repositoryRoot, commandInput) {
      ...
      const workingDirectory = resolveInside(
        await createBoundary(repositoryRoot),
        command.workingDirectory,
      );
      ...
      const timedOut = (execution.error as NodeJS.ErrnoException | undefined)?.code === "ETIMEDOUT";
      const tooLarge = (execution.error as NodeJS.ErrnoException | undefined)?.code === "ENOBUFS";
...
function dockerEnvironment(configurationDirectory: string): NodeJS.ProcessEnv {
  return {
    PATH: process.env.PATH,
    SystemRoot: process.env.SystemRoot,
    PATHEXT: process.env.PATHEXT,
...
export function hardenedContainerProfileFailures(inspected: unknown): string[] {
  ...
  const emptyEntrypoint =
    entrypoint === null || (Array.isArray(entrypoint) && entrypoint.length === 0);
  const noNewPrivileges = securityConfiguration.some(
    (item) => item === "no-new-privileges" || item === "no-new-privileges=true",
  );
...
async function runBoundedProcess(
  ...
  return new Promise((resolve) => {
    ...
    const inspect = (kind: "stdout" | "stderr", chunk: Buffer) => {
...
export function createContainerSandboxRunner(options: ContainerRunnerOptions): VerificationRunner {
  const docker = options.dockerExecutable ?? "docker";
  let cachedCapability: SandboxCapability | undefined;

  const inspectCapability = async (): Promise<SandboxCapability> => {
    ...
    try {
      ...
      let runtimeInfo:
        | {
            readonly OSType?: unknown;
            readonly MemoryLimit?: unknown;
            readonly PidsLimit?: unknown;
            readonly SecurityOptions?: unknown;
          }
        | undefined;
      try {
        runtimeInfo = JSON.parse(info.stdout) as typeof runtimeInfo;
      } catch {
      ...
      const securityOptions = Array.isArray(runtimeInfo.SecurityOptions)
        ? runtimeInfo.SecurityOptions.filter((item): item is string => typeof item === "string")
        : [];
      ...
      const runArguments = buildContainerArguments(
        options.image,
        probeContainerName,
        probeWorkspace,
        {
      ...
      const probe = profileReceived
        ? await runBoundedProcess(
            docker,
            ["start", "--attach", probeContainerName],
            environment,
            15_000,
            () => {
    ...
    async run(repositoryRoot, commandInput) {
      ...
      if (capability.state !== "AVAILABLE_HARDENED") {
        return verificationExecutionResultSchema.parse({
          ...
          runner: "CONTAINER_SANDBOX",
          state:
            capability.state === "UNAVAILABLE" ? "SANDBOX_UNAVAILABLE" : "SANDBOX_MISCONFIGURED",
